...
<div class="container">
	<h2>EASY</h2>
		...
		$sqlquery="SELECT `B`.`Company_id`, `B`.`Enrollement_id`, `B`.`Card_id`, `B`.`First_name`, `B`.`Last_name`, `B`.`Current_address`, `B`.`Phone_no`, `B`.`User_email_id`, `A`.`OPT_code`, `A`.`Varifed` FROM `igain_sent_otp` as `A` JOIN `igain_enrollment_master` AS `B` ON `A`.`Beneficiary_membership_id` = `B`.`Card_id` WHERE `A`.`Igain_company_id` = '7'"; 
