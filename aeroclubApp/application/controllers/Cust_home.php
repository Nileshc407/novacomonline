...
class Cust_home extends CI_Controller {
	...
	{	
        if ($this->session->userdata('cust_logged_in')) {
			...
			$data["BrandImages"] = $this->Igain_model->get_brand_images($session_data['Company_id'],$_SESSION['brndID']); 
			 ...
			 // $phonecode = $Country_details->phonecode;
			// die;
		...
		//------------------Qr Code-------------------//
			...
			$params['level'] = 'H';
			$params['size'] = 5;
			$params['savename'] = FCPATH . 'qr_code_profiles/' . $data['enroll'].'.png';
			$QrCode_image = $this->ciqrcode->generate($params);
		//---------------------Qr Code--------------//	
	...
	{  
        if ($this->session->userdata('cust_logged_in')) {
			...
			if($_POST==null){
				...
				$homeEmailId = App_string_encrypt($this->input->post('homeEmailId')); 
				...
				$currentAddress = App_string_encrypt($this->input->post('currentAddress')); 
	...
	public function check_email_id1() 
	{
		$enc_email = App_string_encrypt($this->input->post("homeEmailId"));
        $result = $this->Igain_model->Check_EmailID1($enc_email, $this->input->post("Company_id"));
        $this->output->set_output($result);
       /*  if ($result == '1') 
		{
            $result = array();
	...
	public function check_phone_number1() 
	{ 
        $phoneNo = $this->input->post("phno1");
        $Company_id = $this->input->post("Company_id");
		// $Country = $this->input->post("Country");
        // $Dial_Code = $this->Igain_model->get_dial_code($Country);
        // $dialcode = $Dial_Code->phonecode;
       // $phoneNo = $dialcode . $phno;
		// echo $phoneNo;
		$phoneNo =  App_string_encrypt($phoneNo);
        $result = $this->Igain_model->CheckPhone_number1($phoneNo, $Company_id);
        $this->output->set_output($result);
	...
	{	
        if ($this->session->userdata('cust_logged_in')) {
			...
			if($Request_flag == 1)
			{
				...
				$data['Msg'] = "Present code below to your server to earn amount";
			...
			else if($Request_flag == 2)
			{
				...
				$data['Msg'] = "Present code below to your server to pay amount";
			...
			{
				$Pin_type = "redeem points";
				$data['Msg'] = "5 Digit Code Generated Sucessfully.";
			}
	...
	public function club()
    {
		...
		if($this->session->userdata('cust_logged_in'))
		{
			...
			$data["Enroll_details"] = $this->Igain_model->get_enrollment_details($data['enroll']);
            $Enroll_details = $data['Enroll_details'];
             $this->load->view('front/dashboard/club', $data);
	...
	public function membership()
    {
		...
		if($this->session->userdata('cust_logged_in'))
		{
			...
			$data["Enroll_details"] = $this->Igain_model->get_enrollment_details($data['enroll']);
            $Enroll_details = $data['Enroll_details'];
             $this->load->view('front/dashboard/membership', $data);
	...
	public function accommodation()
    {
		...
		if($this->session->userdata('cust_logged_in'))
		{
			...
			$data["Enroll_details"] = $this->Igain_model->get_enrollment_details($data['enroll']);
            $Enroll_details = $data['Enroll_details'];
             $this->load->view('front/dashboard/accommodation', $data);
	...
	public function facilities()
    {
		...
		if($this->session->userdata('cust_logged_in'))
		{
			...
			$data["Enroll_details"] = $this->Igain_model->get_enrollment_details($data['enroll']);
            $Enroll_details = $data['Enroll_details'];
             $this->load->view('front/dashboard/facilities', $data);
	...
	public function more()
    {
		...
		if($this->session->userdata('cust_logged_in'))
		{
			...
			$data["Enroll_details"] = $this->Igain_model->get_enrollment_details($data['enroll']);
            $Enroll_details = $data['Enroll_details'];
             $this->load->view('front/dashboard/more', $data);
	...
	{ 
		...
		/*******************Nilesh Redirect URL  09-07-2020***************************/
			$this->load->view('front/profile/add_balance', $data);
		}
		else
		{
			redirect('Login', 'refresh');
	...
	public function CheckoutBalancePayment()
	{
		if($this->session->userdata('cust_logged_in'))
		{
			$session_data = $this->session->userdata('cust_logged_in');
			...
			$Seller_name = $Enroll_details->First_name.' '.$Enroll_details->Middle_name.' '.$Enroll_details->Last_name;
			$Purchase_Bill_no = $Enroll_details->Topup_Bill_no;
			$tp_db = $Purchase_Bill_no;
			$len = strlen($tp_db);
			$str = substr($tp_db,0,5);
			$bill = substr($tp_db,5,$len);
			...
			$Total_topup_amt = $Enroll_details1->Total_topup_amt;
			...
			/**********************Nilesh VerifyToken 09-07-2020****************************/	
				...
				$Paid_amount = $_REQUEST['Paid_by_MPESA_Amount'];
				...
				if($Paid_amount > 0 && ($Seller_api_url2 != "" || $Seller_api_url2 != NULL))
				{
					...
					$input_args3 = Array('TransID' => $Mpesa_TransID ,'BalanceDue' => number_format($Mpesa_Total_Paid_Amount,2),'PaidAmount' => number_format($Mpesa_Total_Paid_Amount,2),'BillRefNumber' => $BillRefNumber,"PaymentMethod"=> "MOBILEMONEY","PaymentType"=> "MPESA");
					...
					if($Mpesa_error_code == '0000')
					{						
						$TopUpPoints = $Paid_amount*$Company_Redemptionratio;
					
						$TopUp = floor($TopUpPoints);
						
						$Update_Current_balance = ($Current_balance + $TopUp);
						
						// $Update_topup_amt = $Total_topup_amt + $TopUp;
						...
						$bill_no = $bill + 1;
						$billno_withyear = $str.$bill_no;
						$result4 = $this->Igain_model->update_topup_billno($Seller_id,$billno_withyear);
						
						$Update_Current_balance = ($Current_balance + $TopUp);
						
						$Update_topup_amt = $Total_topup_amt + $TopUp;
						
						$up = array('Current_balance' => $Update_Current_balance,'Total_topup_amt' => $Update_topup_amt);
						...
						$Email_content = array(
							  ...
							  'Symbol_currency' => $data['Symbol_of_currency'],
							  'TopUp' => $TopUp,
							  'Paid_by_MPESA' => $_REQUEST['Paid_by_MPESA_Amount'],
							  'Paid_by_CreditCard' => $DPOTransFinalAmount1,
							  'Paid_amount' => number_format($Paid_amount, 2),
							  'Notification_type' => 'Add Balance',
							  'Template_type' => 'Add_balance'
						  );
						
						$Notification=$this->send_notification->send_Notification_email($data['enroll'],$Email_content,$Seller_id,$data['Company_id']); 
						
						$this->session->set_flashdata("error_code","Balance Added Successfully!!");
						$Msg_code = $Mpesa_error_code;
					}
					else
					{
						$Msg_code = $Mpesa_error_code;
						
						$this->session->set_flashdata("error_code","Payment not Received..Try Again!");
					}
				}
			...
			$data['Bill_no'] = $BillRefNumber;
			$data['Top_up'] = $TopUp;
			$data['Paid_amount'] = number_format($Paid_amount, 2);
			$data['Current_balance'] = $Update_Current_balance;
			
			$this->load->view('front/home/add_balance_message', $data);
		}
		else
		{
			redirect('Login', 'refresh');
	...
	{ 
		if($this->session->userdata('cust_logged_in'))
		{
			...
			$RedirectURL = base_url().'index.php/Cust_home/AddBalance';
			$BackURL = base_url().'index.php/Cust_home/AddBalance';
			...
			$ServiceDescription = "Add Balance";
	...
	{ 
		...
		/*******************Nilesh Redirect URL 09-07-2020***************************/
			...
			$data['DPOCreditAmt'] = $_SESSION['DPOCreditAmt']; 
		/*******************Nilesh Redirect URL  09-07-2020***************************/
			$this->load->view('front/profile/add_wallet_balance', $data);
		}
		else
		{
			redirect('Login', 'refresh');
	...
	public function CheckoutWalletBalancePayment()
	{
		if($this->session->userdata('cust_logged_in'))
		{
			$session_data = $this->session->userdata('cust_logged_in');
			...
			$Wallet_balance = $Enroll_details1->Wallet_balance;
			$Total_topup_amt = $Enroll_details1->Total_wallet_topup;
			...
			/**********************Nilesh VerifyToken 09-07-2020****************************/
				...
				if($TransactionToken != Null && $CCDapproval != Null) 
				{
					...
					else 
					{
						...
						{
							 redirect('Cust_home/AddWalletBalance');
						}
					}
				}
			/**********************Nilesh VerifyToken 09-07-2020****************************/	
				
				$Trans_type = 30; // Wallet Credit
				$Payment_type_id = 5; //MPesa
				$Remarks = "Wallet Credit";
				$Paid_by = "MPesa";  
				// $Mpesa_TransID = $_REQUEST['Trans_id'];
				$Mpesa_TransID = $_REQUEST['Mpesa_TransID'];
				$Date = date("Y-m-d");
				...
				if($Paid_amount > 0 && ($Seller_api_url2 != "" || $Seller_api_url2 != NULL))
				{
					...
					if($Mpesa_error_code == '0000')
					{
						...
						$Update_Current_balance = ($Wallet_balance + $TopUp);
						...
						$data123 = array('Company_id' => $data['Company_id'],
											...
											'Online_payment_method' => $Paid_by,
											'Credit_Cheque_number' => $DPOTransactionApproval1,
											'Available_wallet_balance' => $Update_Current_balance
										);	
						...
						$Update_Wallet_balance = ($Wallet_balance + $TopUp);
						
						$Update_topup_amt = $Total_topup_amt + $TopUp;
						
						$up = array('Wallet_balance' => $Update_Wallet_balance,'Total_wallet_topup' => $Update_topup_amt);
						...
						$this->session->set_flashdata("error_code","Wallet Balance Added successfully!!");
					}
					else
					{
						$this->session->set_flashdata("error_code","Payment Not Received.. Please try Again!");
					}
				}
				else if($CCDapproval != Null)
				{
			...
			$data['Wallet_balance'] = $Update_Wallet_balance;
			
			$this->load->view('front/home/add_wallet_balance_message', $data);
		}
		else
		{
			redirect('Login', 'refresh');
