...
class Igain_model extends CI_Model
{
	...
	function Check_EmailID1($emailId,$Company_id)
	{		
		$query =  $this->db->select('User_email_id1')
				   ->from('igain_enrollment_master')
				   ->where(array('User_email_id1' => $emailId,'User_id'=>1,'Company_id' => $Company_id))->get();
				    // echo $this->db->last_query();
			return $query->num_rows();
	}
	...
	function CheckPhone_number1($phoneNo,$Company_id)
	{
		
		$query =  $this->db->select('Phone_no1')
				   ->from('igain_enrollment_master')
				   ->where(array('Phone_no1' => $phoneNo ,'User_id'=>1,'Company_id' => $Company_id))->get();
				   // echo $this->db->last_query();
			return $query->num_rows();
	}
   ...
   public function forgot_email_notification($email,$Company_id)
	{
		...
		$this->db->from('igain_enrollment_master');
		$this->db->where(array('Company_id' => $Company_id,'User_email_id' => $email,'User_id' =>'1','User_activated' =>'1'));	
		$query = $this->db->get();
...
/********************************************Akshay Start*******************************/
	...
	{
		$this->db->select("*,A.Trans_type as TransType,SUM(A.Loyalty_pts) as Loyalty_pts,SUM(A.Redeem_points) as Redeem_pts,SUM(A.Purchase_amount) as Purchase_amt,CONCAT(B.First_name,' ',B.Last_name) as Outlet_name,B.Photograph as Outlet_logo,A.Topup_amount,A.Paid_amount");
		$this->db->from("igain_transaction as A");
		$this->db->where(array('A.Company_id'=>$Company_id, 'A.Enrollement_id'=>$enroll_id));
		$this->db->where("A.Trans_type IN('1','2','3','30','31')");
		$this->db->join("igain_enrollment_master as B", "A.Seller=B.Enrollement_id");
		$this->db->order_by('A.Trans_id','desc');
	...
	function get_company_smtp_details($Company_id)
	{
		$this->db->select('*');
