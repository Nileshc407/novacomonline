...
class Send_notification 
{
    ...
    public function send_Notification_email($Enrollment_id,$Email_content,$Seller_id,$Company_id)
    {
		...
		if($Template_type == "Enroll")
        {
			...
			ob_end_clean();			
				...
				// var_dump($myData);
				$Pwddata = base64_encode(json_encode($myData));
				$Pwddata_URL = $Base_url2. "javahouseafrica/index.php/Login/Setpassword?Pwd_data=" . $Pwddata;
				// $PwddataLink = "<a href='" . $Pwddata_URL . "' target='_blank' style='color:#000;'>Click here to Set Password</a>";
				$PwddataLink =$Pwddata_URL;
				
			/************************************Variable Replace Code******************************/
				$search_variables = array('$First_name','$Customer_name','$Merchant_name','$Membership_ID','$Enrolled_under','$User_email_id','$Phone_no','$User_pwd','$Pwdlink','$pinno','$Pin_no_applicable','$Member_website','$Company_name','$Gooogle_Play','$iOs_app_store','$Logo');//
				$inserts_contents = array($First_name,$Customer_name,$Merchant_name,$Membership_ID,$Email_content['Enrolled_under'],$User_email_id,$Phone_no,$User_pwd,$PwddataLink,$pinno,$Pin_no_applicable,$Member_website,$Company_name,$Gooogle_Play,$iOs_app_store,$Logo);
				$html = str_replace($search_variables,$inserts_contents,$body);
			/************************************Variable Replace Code******************************/
		...
		if($Template_type == "Forgot_password")
        {
			...
			// var_dump($myData);
			$Pwddata = base64_encode(json_encode($myData));
			$Pwddata_URL = $Base_url2. "aeroclub/index.php/Login/Setpassword?Pwd_data=" . $Pwddata;
			$PwddataLink = "<a href='" . $Pwddata_URL . "' id='OpenUrl' target='_blank' style='color:#000;'>Click here to Set Password</a>";
			/************************************Variable Replace Code******************************/
				$search_variables = array('$First_name','$Customer_name','$Membership_ID','$User_email_id','$Phone_no','$User_pwd','$pinno','$Pwdlink','$Member_website','$Company_name','$Gooogle_Play','$iOs_app_store','$Logo');//
				$inserts_contents = array($First_name,$Customer_name,$Membership_ID,$User_email_id,$Phone_no,$User_pwd,$pinno,$PwddataLink,$Member_website,$Company_name,$Gooogle_Play,$iOs_app_store,$Logo);
				$html = str_replace($search_variables,$inserts_contents,$body);
			/************************************Variable Replace Code******************************/
		}
	...
	/**************16-05-2020 nilesh purchase gift card**********************/
		...
		{  
			...
			$TopUp = $Email_content['TopUp'];
			$Paid_by_MPESA = $Email_content['Paid_by_MPESA'];
			$Paid_by_CreditCard = $Email_content['Paid_by_CreditCard'];
			$Paid_amount = $Email_content['Paid_amount'];
			$Customer_name = ucwords($customer_details->First_name).' '.ucwords($customer_details->Last_name);
			$First_name = ucwords($customer_details->First_name);
			$pin=$Email_content['pin'];
			$Logo = $Base_url."/assets/images/logo.png";
			ob_start();
			include './application/libraries/Email_templates/Add_balance.php';

			$body = ob_get_contents();
			ob_end_clean();

			/************************************Variable Replace Code***************************** */
			$search_variables = array('$First_name','$Customer_name', '$Symbol_currency', '$Company_name','$Logo','$Transaction_date','$Bill_no','$TopUp','$Paid_by_MPESA','$Paid_by_CreditCard','$Paid_amount','$Current_point_balance','$Company_Currency'); //
			$inserts_contents = array($First_name, $Customer_name, $Symbol_currency,$Company_name,$Logo,$Transaction_date,$Bill_no,$TopUp,$Paid_by_MPESA,$Paid_by_CreditCard,$Paid_amount,$Current_point_balance,$Company_Currency);
			$html = str_replace($search_variables, $inserts_contents, $body);
			/************************************Variable Replace Code***************************** */
			// echo $html; die;
		}
		...
		{  
			$Communication_id = 0;
			$Offer = "Add Wallet Balance";
			
			$subject = "Add Wallet Balance";
			...
			$Logo = $Base_url."/assets/images/logo.png";
			ob_start();
			include './application/libraries/Email_templates/Add_wallet_balance.php';

			$body = ob_get_contents();
			ob_end_clean();

			/************************************Variable Replace Code***************************** */
			$search_variables = array('$First_name','$Customer_name', '$Symbol_currency', '$Company_name','$Logo','$Transaction_date','$Bill_no','$TopUp','$Paid_by_MPESA','$Paid_by_CreditCard','$Paid_amount','$Current_point_balance','$Wallet_balance','$Company_Currency'); //
			$inserts_contents = array($First_name, $Customer_name, $Symbol_currency,$Company_name,$Logo,$Transaction_date,$Bill_no,$TopUp,$Paid_by_MPESA,$Paid_by_CreditCard,$Paid_amount,$Current_point_balance,$Wallet_balance,$Company_Currency);
			$html = str_replace($search_variables, $inserts_contents, $body);
			/************************************Variable Replace Code***************************** */
			// echo $html; die;
		}
        ...
        //**************************Email Fuction Code*****************************
			...
			{						
				...
				$mail->addAddress($Partner_contact_person_email, $Customer_name);
			...
			if($Template_type == "Purchase_order"){
				...
				$this->bcc($company_details->Company_contactus_email_id);	
			}
