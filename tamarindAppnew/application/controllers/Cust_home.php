...
class Cust_home extends CI_Controller {
	...
	{	 
        if ($this->session->userdata('cust_logged_in')) {
			...
			/* 28-12-2020 */
				$data['Verification_code'] = $this->Igain_model->check_verification_code($session_data['Company_id'],$session_data['enroll']);check_verification_code
			/* 28-12-2020 */
		...
		if(filter_var($email, FILTER_VALIDATE_EMAIL)) {
			...
			if(preg_match("/^[0-9]{9}+$/", $Phone_no)) {
				...
				if($password_len < 6 ){
					...
					echo json_encode(array("status" => "1003", "Message" => "Password should be minimum of 6 characters"));
					exit;					
				}
				...
				if(strpos($password, '#') !== false || strpos($password, '%') !== false || strpos($password, '&') !== false || strpos($password, '+') !== false || strpos($password, '=') !== false ){
					...
					echo json_encode(array("status" => "1008", "Message" => "Password should not allow #,%,&, + and = characters"));
					exit;
				}			
				if($password_len < 6 ){
					
					echo json_encode(array("status" => "1003", "Message" => "Password should be minimum of 6 characters with one number, one special, one upper and one lower case letter"));
					exit;					
				}
				if($password_upper_char != 1 ){
					
					echo json_encode(array("status" => "1004", "Message" => "Password should be minimum of 6 characters with one number, one special, one upper and one lower case letter"));
					exit;					
				}
				if($password_lower_char != 1 ){
					
					echo json_encode(array("status" => "1005", "Message" => "Password should be minimum of 6 characters with one number, one special, one upper and one lower case letter"));
					exit;					
				}
				if($password_special_char != 1 ){
					
					echo json_encode(array("status" => "1006", "Message" => "Password should be minimum of 6 characters with one number, one special, one upper and one lower case letter"));
					exit;					
				}
				if($password_numaric != 1 ){
					
					echo json_encode(array("status" => "1007", "Message" => "Password should be minimum of 6 characters with one number, one special, one upper and one lower case letter"));
					exit;					
				}
				*/				
				...
				if($matchepwd != 0){
					...
					echo json_encode(array("status" => "1009", "Message" => "Password and confirm password don't match"));
					exit;
					
				} else if ($flag == NULL && $CompanyID == NULL && $email == NULL && $Phone_no == NULL) {
					
					echo json_encode(array("status" => "1010", "Message" => 'Provide valid information. Please
					try again'));
					exit;
					   ...
					   // echo "---Currentyear-----".$Currentyear."---<br>";
						...
						if ($YearDiff < 18 ) {
							echo json_encode(array("status" => "1011", "Message" => 'Must be 18+ to be eligible for Loyalty'));
							exit;
						 } */                  


						if($first_name != "" && $last_name != "" && $email != "" && $Company_id != "" && $Phone_no != "" && $password != "") {
						...
						if ($Check_EmailID == 0) {
							...
							if ($CheckPhone_number == 0) {
								...
								echo json_encode(array("status" => "1014", "Message" => 'The provided phone number already exists. Please try another phone number'));
								exit;
							}
						} else {
							echo json_encode(array("status" => "1015", "Message" => 'The provided email address already exists. Please try another email address'));
							exit;
						}
						
					} else {
					   
						echo json_encode(array("status" => "1016", "Message" => 'Provide valid information. Please try again'));
						exit;
				...
				echo json_encode(array("status" => "1002", "Message" => 'Enter valid phone number'));
				exit;
			...
			echo json_encode(array("status" => "1001", "Message" => 'Please enter a valid email address'));
			exit;
	   ...
	   if(filter_var($email, FILTER_VALIDATE_EMAIL)) {
			...
			if ( !empty($email) &&  !empty($Company_id)) {
				...
				if($result != NULL || $result > 0) {
					...
					echo json_encode(array("status" => "1022", "Message" => "Login credentials are sent to your email. Please check."));
					exit;
					
				} else {
					
					echo json_encode(array("status" => "1023", "Message" => "The provided email doesn't match our records"));
					exit;
				}
				
			} else {
				
				echo json_encode(array("status" => "1019", "Message" => "Enter valid email address"));
				exit;
			}
		} else {
	...
	function Verified_email() 
	{
        if ($this->session->userdata('cust_logged_in')) {
			...
			$userEmailId = strtolower($userEmailId);
		...
		if(filter_var($email, FILTER_VALIDATE_EMAIL)) {
			...
			if (!empty($email) && !empty($Company_id)) {
				...
				if($result > 0) {               
					...
					echo json_encode(array("status" => "1021", "Message" => "The provided email address already exists in the system, Please try another email"));
					exit;
					
				} else {
					
					echo json_encode(array("status" => "1020", "Message" => "The provided email is available"));
					exit;
				}
			} else {
				echo json_encode(array("status" => "1019", "Message" => "Enter valid email address"));
		...
		if(preg_match("/^[0-9]{9}+$/", $Phone_no)) {
			...
			if ($is_numeric == true && !empty($Phone_no) && strlen($Phone_no) == 9 && $Company_id != "") {
				...
				if($ValidatePhone_number > 0) {               
					...
					echo json_encode(array("status" => "1014", "Message" => "The provided phone number already exists in the system, Please try another phone number"));
					exit;
					
				} else {
					
					echo json_encode(array("status" => "1017", "Message" => "The provided phone number is available"));
					exit;
				}
				
			} else {
				echo json_encode(array("status" => "1018", "Message" => "Enter valid phone number"));
				exit;
			}
