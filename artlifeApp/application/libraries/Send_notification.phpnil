...
class Send_notification 
{
    ...
    public function send_Notification_email($Enrollment_id,$Email_content,$Seller_id,$Company_id)
    {
		...
		if($Template_type == "Enroll")
        {
			...
			/************************************Variable Replace Code******************************/
				$search_variables = array('$First_name','$Customer_name','$Merchant_name','$Membership_ID','$Enrolled_under','$User_email_id','$Phone_no','$User_pwd','$Pwdlink','$pinno','$Pin_no_applicable','$Member_website','$Company_name','$Gooogle_Play','$iOs_app_store');//
				$inserts_contents = array($First_name,$Customer_name,$Merchant_name,$Membership_ID,$Email_content['Enrolled_under'],$User_email_id,$Phone_no,$User_pwd,$PwddataLink,$pinno,$Pin_no_applicable,$Member_website,$Company_name,$Gooogle_Play,$iOs_app_store);
				$html = str_replace($search_variables,$inserts_contents,$body);
			/************************************Variable Replace Code******************************/
			 // echo $html; die;
        }		
	...
	/**************16-05-2020 nilesh purchase gift card**********************/
		...
		{  
			...
			/************************************Variable Replace Code***************************** */
			$search_variables = array('$First_name','$Customer_name', '$pin', '$Company_name'); //
			$inserts_contents = array($First_name, $Customer_name, $pin,$Company_name);
			$html = str_replace($search_variables, $inserts_contents, $body);
			/************************************Variable Replace Code***************************** */
			// echo $html; die;
		}
		...
		if($Enrollement_id != 0 && $customer_details->User_id == 1 && $Template_type != "Contactus" && $Template_type != "Purchase_item_return_initiated_to_admin" && $Template_type != "From_transfer_miles"&& $Template_type != "To_transfer_miles" && $Template_type != "Email_verification")
		{
			if($Template_type == "Purchase_order_to_company") 
			{
				$Enrollement_id = 0;
        ...
        //**************************Email Fuction Code*****************************
			...
			if($Template_type == "Email_verification" || $Template_type == "Forgot_password" || $Template_type == "Contactus")
			{
