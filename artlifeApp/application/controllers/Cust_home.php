...
class Cust_home extends CI_Controller {
	...
	function points_history() 
	{
        if ($this->session->userdata('cust_logged_in')) {
			...
			// if($_GET['brndID'])
				$brandStampDetailsNew= array();
			foreach($data['brandDetails'] as $key => $value)
			{
				 // echo"---Brand---".$value['Enrollement_id']."---<br>";
				 
				// $data['brndID']=$_GET['brndID'];
				// $data['brndID']=$value['Enrollement_id'];
				// $data['Sub_Seller_details'] = $this->Igain_model->Fetch_Seller_outlet_details($data['Company_id'],$_GET['brndID']);
				$data['Sub_Seller_details'] = $this->Igain_model->Fetch_Seller_outlet_details($data['Company_id'],$value['Enrollement_id']);
				
				$enroll= array();
				// $enroll[$_GET['brndID']]=$_GET['brndID'];
				// $enroll[$value['Enrollement_id']]=$value['Sub_seller_Enrollement_id'];
				
				foreach($data['Sub_Seller_details'] as $SS_details){
					 // echo"---Enrollement_id--SS_details---".$SS_details['Enrollement_id']."---<br>";
					$enroll[$SS_details['Enrollement_id']]=$SS_details['Enrollement_id'];
					
				}
				// $a=array("red","green");
				// $enrollids=array_push($enrollids,$_GET['brndID']);
				$enroll[$value['Enrollement_id']]=$value['Enrollement_id'];
				// $enroll[$value['Enrollement_id']]=123;
				// print_r($enroll[$SS_details['Enrollement_id']]);
				// $data['brandStampDetails'] = $this->Igain_model->Fetch_Seller_Stamp_details($data['Company_id'],$data['enroll'],$enroll);
				$brandStampDetailsNew[] = $this->Igain_model->Fetch_Seller_Stamp_details($data['Company_id'],$data['enroll'],$enroll);
				$data['brandStampDetails']=$brandStampDetailsNew;
			...
			$_SESSION['brndID']=0;
			// die;
            $this->load->view('front/dashboard/view_points', $data);
        } else {
            redirect('login', 'refresh');
        }
    }
