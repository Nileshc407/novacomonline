...
class Send_notification 
{
    ...
    public function send_Notification_email($Enrollment_id,$Email_content,$Seller_id,$Company_id)
    {
		...
		$smtp_details = $this->CI->Igain_model->get_smtp_details($Company_id);
		if($smtp_details != Null){
			...
			$smtp_details1 = $this->CI->Igain_model->get_smtp_details(3);
			$smtp_host = $smtp_details1->smtp_host;
			$smtp_user = $smtp_details1->smtp_user;
			$smtp_pass = $smtp_details1->smtp_pass;
			$smtp_port = $smtp_details1->smtp_port;
		}
		/*--------------Encripted & Decrypted------------------------------------------*/
		$User_email_id_cont = App_string_decrypt($User_email_id_cont); 
		$User_email_id = App_string_decrypt($User_email_id);
		$Cust_email_id = $User_email_id;
		...
		if($Template_type == "Forgot_password")
        {
			...
			// $Pwddata_URL = $Base_url2. "javahouseafrica/index.php/Login/Setpassword?Pwd_data=" . $Pwddata;
			$Pwddata_URL = "https://javahouseafrica.ehp.online/index.php/Login/Setpassword?Pwd_data=" . $Pwddata;
			
			$PwddataLink = "<u><a href='" . $Pwddata_URL . "' id='OpenUrl' target='_blank' style='color:#000;'>Click here to Set Password</a></u>";
			/************************************Variable Replace Code******************************/
		...
		if($Template_type == "Member_verification_otp")
        {
			$Communication_id = 0;
			$Offer = $Email_content['Notification_type'];			
			...
			$subject = "OTP for Java House Africa Loyalty APP";		    
			include './application/libraries/Email_templates/Member_verification_otp.php';			
					
			$body = ob_get_contents();
			ob_end_clean();	
			
			/************************************Variable Replace Code******************************/
				$search_variables = array('$Customer_name','$OTP','$Company_name','$Logo');//
				$inserts_contents = array($Customer_name,$OTP,$company_details->Company_name,$Logo);
				$html = str_replace($search_variables,$inserts_contents,$body);
			/************************************Variable Replace Code******************************/
			 //echo $html;
		}
	 ...
	 if($Template_type == "From_transfer_miles")
		...
		{  
			...
			$DeliveryOutlet = $Email_content['Outlet_id'];
			$delivery_outlet = $Email_content['Outlet_name'];
			...
			include './application/libraries/Email_templates/Pos_Purchase_order.php';	
		...
		if($Template_type == "Contactus")
        {
			...
			/************************************Variable Replace Code******************************/
				$search_variables = array('$Customer_name','$Cust_email_id','$Company_primary_contact_person','$Offer','$Notification_description','$Membership_ID','$Phone_no','$Member_website','$Company_name','$Gooogle_Play','$iOs_app_store');//
				$inserts_contents = array($Customer_name,$Cust_email_id,$company_details->Company_primary_contact_person,$Offer,$Email_content['Notification_description'],$Membership_ID,$Phone_no,$Member_website,$Company_name,$Gooogle_Play,$iOs_app_store);
				$html = str_replace($search_variables,$inserts_contents,$body);
			/************************************Variable Replace Code******************************/
		}
		if($Template_type == "Contactus_feedback")
        {
			...
			/************************************Variable Replace Code******************************/
				$search_variables = array('$Customer_name','$Company_primary_contact_person','$Offer','$Notification_description','$Membership_ID','$Phone_no','$Member_website','$Company_name','$Gooogle_Play','$iOs_app_store','$Logo');//
				$inserts_contents = array($Customer_name,$company_details->Company_primary_contact_person,$Offer,$Email_content['Notification_description'],$Membership_ID,$Phone_no,$Member_website,$Company_name,$Gooogle_Play,$iOs_app_store,$Logo);
				$html = str_replace($search_variables,$inserts_contents,$body);
			/************************************Variable Replace Code******************************/			
		}
	...
	/**************16-05-2020 nilesh purchase gift card**********************/
        ...
        //**************************Email Fuction Code*****************************
			...
			if($Template_type == "Email_verification" || $Template_type == "Forgot_password" || $Template_type == "Send_gift_card" || $Template_type == "Enroll" || $Template_type == "Member_verification_otp" || $Template_type == "Contactus")
			{	
				if(!$mail->send()) 
				{  
					// echo"Mail Send";
					// echo "print_debugger--------".$mail->print_debugger();              
