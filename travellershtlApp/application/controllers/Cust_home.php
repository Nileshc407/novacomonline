...
class Cust_home extends CI_Controller {
                        ...
                        if ($Membership_id == $Enroll_details12->Card_id || $Membership_id == $Enroll_details12->Phone_no) {
						   ...
						   $Transfer_PTS = array(
                            ...
                            if ($result > 0) {
                                ...
                                $data["Success_Message"] = $Company_details->Currency_name." Transfered Successfully";
                                $this->load->view('front/transferpoints/success', $data);
                            }
                        }
                    } else {
	...
	{ 
		if($this->session->userdata('cust_logged_in'))
		{
			...
			$this->load->view('front/home/buy_gift_card', $data);
		}
		else
		{
			redirect('Login', 'refresh');
	...
	{ 
		...
		{ 
			...
			$data['Gift_card_amt'] = $_REQUEST["Gift_card_amt"];
	...
	public function CheckoutGiftCardPayment()
	{
		if($this->session->userdata('cust_logged_in'))
		{
			...
			$data['username'] = $session_data['username'];			
			$data['enroll'] = $session_data['enroll'];
			$cust_enrollment_id = $session_data['enroll'];
			$data['userId']= $session_data['userId'];
			$data['Card_id']= $session_data['Card_id'];
			$data['Company_id']= $session_data['Company_id'];
			...
			$Cust_current_bal = $Enroll_details1 -> Current_balance;
			$Cust_Blocked_points = $Enroll_details1 -> Blocked_points;
			$Cust_Debit_points = $Enroll_details1 -> Debit_points;
			$Total_reddems = $Enroll_details1 -> Total_reddems;

			$Current_point_balance = $Cust_current_bal-($Enroll_details1->Blocked_points+$Enroll_details1->Debit_points);					
			...
			if($_POST != NULL)
			{
				...
				$Gift_card_amt = $_REQUEST['Gift_card_amt'];
				...
				if($Point_redeem > 0){
					if($Current_point_balance < $Point_redeem)
					{
						$error_code = "Insufficient Current Balance!";
						$error_flag = 1;
					}
				}
				if($Paid_amount > 0 && ($data['Seller_api_url2'] != "" || $data['Seller_api_url2'] != NULL))
				{
					...
					$url = $data['Seller_api_url2']."ConfirmB2BPayment";					
					...
					$newHash = $payment_auth_key; 
						...
						if($Mpesa_error_code == '0000')
						{
							...
							$data123 = array('Company_id' => $data['Company_id'],
												...
												'Redeem_points' => $Point_redeem,
												'Redeem_amount' => $Redeem_amt,
							...
							$Update_Current_balance = $Cust_current_bal - $Point_redeem;
							$Update_reddems = $Total_reddems + $Point_redeem;
							
							$up = array('Current_balance' => $Update_Current_balance, 'Total_reddems' => $Update_reddems);
										
							$this->Loyalty_model->update_cust_balance($up,$cust_enrollment_id,$data['Company_id']);	
									
							$error_code="Thanks for your Gift Card Purchase!";
							$error_flag = 0;
						}
						else
						{
							$error_code = "please provide valid inputs!!"; 
							$error_flag = 1;
						}
				}
				else if($Gift_card_amt == $Redeem_amt)
				{
					$Payment_type_id = 4;
	...
	{	
        if ($this->session->userdata('cust_logged_in')) {
			...
			$this->load->view('front/dashboard/member_benefits', $data);
				
        } else {
            redirect('login', 'refresh');
        }	
    }
	public function work_earn_points()
	{	
        if ($this->session->userdata('cust_logged_in')) {
            $session_data = $this->session->userdata('cust_logged_in');
            $data['username'] = $session_data['username'];
			...
			$this->load->view('front/dashboard/work_earn_points', $data);
				
        } else {
            redirect('login', 'refresh');
        }	
    }
	public function work_redeem_points()
	{	
        if ($this->session->userdata('cust_logged_in')) {
            $session_data = $this->session->userdata('cust_logged_in');
            $data['username'] = $session_data['username'];
			...
			$this->load->view('front/dashboard/work_redeem_points', $data);
				
        } else {
            redirect('login', 'refresh');
        }	
    }
	public function work_collect_stamps()
	{	
        if ($this->session->userdata('cust_logged_in')) {
            $session_data = $this->session->userdata('cust_logged_in');
            $data['username'] = $session_data['username'];
			...
			$this->load->view('front/dashboard/work_collect_stamps', $data);
