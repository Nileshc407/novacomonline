...
class Igain_model extends CI_Model
{
	...
	function validateRandomCode($Company_id)
	{
		$code = $this->getRandomCode();
		$pin_exist = $this->check_random_code($code,$Company_id);
		if($pin_exist > 0)
		{
			$this->validateRandomCode($Company_id);
		}
		else
		{
			return $code;
	...
	function check_random_code($code,$Company_id)
	{
		$query =  $this->db->select('Enrollement_id')
				   ->from('igain_enrollment_master')
				   ->where(array('pinno' => $code, 'Company_id' => $Company_id, 'User_id' => 1))->get();
...
/********************************************Akshay Start*******************************/
	...
	{
		$this->db->select("*,A.Trans_type as TransType,SUM(A.Loyalty_pts) as Loyalty_pts,SUM(A.Redeem_points) as Redeem_pts,SUM(A.Purchase_amount) as Purchase_amt,CONCAT(B.First_name,' ',B.Last_name) as Outlet_name,B.Photograph as Outlet_logo,A.Expired_points");
		$this->db->from("igain_transaction as A");
		$this->db->where(array('A.Company_id'=>$Company_id, 'A.Enrollement_id'=>$enroll_id));
		$this->db->where("A.Trans_type IN('2','12','29','14')");
		$this->db->join("igain_enrollment_master as B", "A.Seller=B.Enrollement_id");
		$this->db->order_by('A.Trans_id','desc');
	...
	function Get_my_gift_cards($Enrollment_id,$Company_id,$Card_id)
	{
		...
		$this->db->from('igain_giftcard_tbl as T');
		$this->db->join('igain_enrollment_master as IE', 'T.Card_id = IE.Card_id','LEFT');
		$this->db->where(array('T.Company_id' => $Company_id, 'T.Card_id' => $Card_id)); 
		$this->db->where_in('T.Payment_Type_id', array('4','5'));
		$this->db->order_by('T.id','DESC');
		$sql = $this->db->get();
		if($sql->num_rows() > 0)
		{
	...
	{
		$todayDate=date("Y-m-d");				
		$this->db->select("gc.Gift_card_id,gc.Valid_till,gc.Company_id,gc.Seller_id,gc.Card_value,gc.Discount_percentage,gc.Card_balance"); //,OF.Offer_name
		$this->db->from('igain_giftcard_tbl as gc'); 				
				
		$this->db->join('igain_company_send_voucher as SC', 'SC.Voucher_code = gc.Gift_card_id AND SC.Company_id = gc.Company_id'); 
		// $this->db->join('igain_offer_master as OF', 'OF.Offer_code = SC.Offer_code AND OF.Company_id = SC.Company_id'); 
		$this->db->where(array('gc.Company_id' => $Company_id,'gc.Card_id' =>$Card_id,'gc.Used_flag' =>0,'SC.Enrollement_id' => $enroll,'gc.Valid_till >= ' => $todayDate));
		$this->db->where("gc.Payment_Type_id IN('99','997','998')");		
	...
	{
		$todayDate=date("Y-m-d");				
		$this->db->select("gc.Gift_card_id,gc.Valid_till,gc.Company_id,gc.Seller_id,gc.Card_value,gc.Discount_percentage,gc.Card_balance,gc.Update_date"); //OF.Offer_name,
		$this->db->from('igain_giftcard_tbl as gc'); 				
				
		$this->db->join('igain_company_send_voucher as SC', 'SC.Voucher_code = gc.Gift_card_id AND SC.Company_id = gc.Company_id'); 
		// $this->db->join('igain_offer_master as OF', 'OF.Offer_code = SC.Offer_code AND OF.Company_id = SC.Company_id'); 
		$this->db->where(array('gc.Company_id' => $Company_id,'gc.Card_id' =>$Card_id,'gc.Used_flag' =>1,'SC.Enrollement_id' => $enroll));	
		$this->db->where("gc.Payment_Type_id IN('99','997','998')");		
		$this->db->order_by("gc.Seller_id",'ASC');
		$this->db->order_by("gc.Create_date",'DESC');
		$this->db->group_by("gc.Gift_card_id",'DESC');
	...
	{
		$todayDate=date("Y-m-d");				
		$this->db->select("gc.Gift_card_id,gc.Valid_till,gc.Company_id,gc.Seller_id,gc.Card_value,gc.Discount_percentage,gc.Card_balance,gc.Update_date");
		$this->db->from('igain_giftcard_tbl as gc'); 				
	...
	/**************SMS 10-11-2021-------------END-------------------***************/
	function Validate_Claim_code($Claim_code,$Company_id)
	{		
		$this->db->select('*');
		$this->db->from('igain_pos_transaction_temp');
		$this->db->where(array('Claim_code' => $Claim_code, 'Company_id' => $Company_id, 'Claimed_flag' => 0));
		$sql51 = $this->db->get();
		if($sql51 -> num_rows() > 0)
		{
			 return $sql51->row();
	...
	function check_claimed_bill_no($Claim_code,$Company_id,$enroll,$Card_id)
	{
		$this->db->select('*');
		$this->db->from('igain_transaction');
		$this->db->where(array('Claim_code' => $Claim_code, 'Company_id' => $Company_id, 'Enrollement_id' =>$enroll,'Card_id' =>$Card_id,'Trans_type' => 2, 'Channel_id' => 0));
		$query = $this->db->get();		
		return $query->num_rows();
	}
	function get_temp_bill_details($Claim_code,$Company_id)
	{		
		$this->db->select('*');
		$this->db->from('igain_pos_transaction_temp');
		$this->db->where(array('Claim_code' => $Claim_code, 'Company_id' => $Company_id, 'Claimed_flag' => 0));
	...
	function get_member_bal($Enrollement_id,$Company_id,$Redemptionratio)
	{
		$this->db->select("*");
		$this->db->from('igain_enrollment_master');
		$this->db->where(array('Enrollement_id' => $Enrollement_id, 'Company_id' => $Company_id));
		...
		{			
			foreach ($query->result() as $row)
			{
			   ...
			   $Points_value = $Current_balance/$Redemptionratio;
			   
               $result[] = array("Current_balance" => $Current_balance, "Points_value" => number_format($Points_value,0));
            }			
			return json_encode($result);
		}
		else
	...
	function get_company_app_info($Company_id)
	{
		$this->db->select("*");
		$this->db->from('igain_company_app_info');
		$this->db->where(array('Company_id' => $Company_id));
	...
	function get_smtp_details($Company_id)
	{
		$this->db->select("*");
		$this->db->from("igain_smtp_master");
		$this->db->where(array('Company_id' =>$Company_id));
		$sql = $this->db->get();
		if($sql->num_rows() > 0)
		{
