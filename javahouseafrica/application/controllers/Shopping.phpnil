...
class Shopping extends CI_Controller 
{
	...
	function index()
	{
		...
		{	
			...
			/*-----------------------Pagination---------------------*/			
				...
				$total_row = $this->Shopping_model->Get_Merchandize_ecommerce_Items_Count($data['Company_id'],0);
			...
			$Redemption_Items = $this->Shopping_model->get_all_items($config["per_page"],$page,$data['Company_id'],$Merchandize_category_id,$Sort_by,$Sort_by_merchant,$Sort_by_brand,$Sort_by_gender,$Sort_by_name,0);
	...
	{  
		...
		{	
			...
			$Selected_outlet=$delivery_session_data['delivery_outlet'];
			
			// $delivery_type=$delivery_session_data['delivery_type'];	
			// $data['delivery_type'] = $delivery_type;	
			// $data['delivery_outlet'] = $delivery_outlet;	
			
			$Selected_outlet_details = $this->Igain_model->get_enrollment_details($Selected_outlet);
			$Outlet_name = $Selected_outlet_details->First_name.' '.$Selected_outlet_details->Last_name;
			$data['Selected_Outlet_name']= $Outlet_name;
			
			// $data["Merchandize_category"] = $this->Redemption_Model->Get_Merchandize_Category($data['Company_id']);
			$data["Merchandize_category"] = $this->Shopping_model->Get_Outlet_Menu_Category($data['Company_id'],$Selected_outlet);
			...
			/*-----------------------Pagination---------------------*/			
                $config = array();
				$config["base_url"] = base_url() . "/index.php/Shopping/MenuItems";
				$total_row = $this->Shopping_model->Get_Merchandize_ecommerce_Items_Count($data['Company_id'],$Selected_outlet);
				$config["total_rows"] = $total_row;
				$config["per_page"] = 12;
				$config["uri_segment"] = 3;        
				$config['next_link'] = 'Next';
			...
			$Redemption_Items = $this->Shopping_model->get_all_items($config["per_page"],$page,$data['Company_id'],$Merchandize_category_id,$Sort_by,$Sort_by_merchant,$Sort_by_brand,$Sort_by_gender,$Sort_by_name,$Selected_outlet);
    ...
    function filter_result()
	{
			...
			$Redemption_Items1 = $this->Shopping_model->get_all_items(0,0,$Company_id,$Sort_category,$Sort_by,0,0,0,'',0);
	...
	{ 
		...
		/**************Nilesh 30-04-2020******************/	
			$data['Sellerdetails'] = $this->Shopping_model->FetchOutletdetails($data['Company_id']);
		/**************Nilesh 30-04-2020******************/	
	...
	{   
		...
		{	
			...
			*************** Revi Product Voucher logic 01-07-2020 **************************/
			...
			$delivery_session_data = $this->session->userdata('delivery_session');
			$data["delivery_type"] = $delivery_session_data['delivery_type'];
			$data["delivery_outlet"] = $delivery_session_data['delivery_outlet'];
				...
				foreach ($cart as $item)
				{
					...
					///**************** sandeep discount logic 27-01-2020 ***************************
						...
						$Item_price = $item['price'] * $item['qty'];
						 ...
						 // echo"---ItemCode---".$ItemCode."------Itemcategory_id---".$Itemcategory_key."------Item_price---".$Item_price."---BrandOutlet_Id-------".$BrandOutlet_Id."<br>"; die;
			...
			//echo "----".$order_sub_total."----".$order_total;
				...
				// echo "<br>---DiscountAmt-11-----".$data["DiscountAmt"]; die;
			}
	...
	{ 
		if($this->session->userdata('cust_logged_in'))
		{
			...
			$this->load->view('Shopping/Select_outlet', $data);
		}
		else
		{
			redirect('Login', 'refresh');
		...
		if($this->session->userdata('cust_logged_in'))
		{
			...
			if($_GET['product_id'] != "")
			{
				...
				if($Product_id_decrypt == ""){
					...
					$data["Product_offers"]=$this->Shopping_model->get_product_offers($Product_id_decrypt,0,$session_data['Company_id']);
...
// 'shipping' => $_SESSION['Total_Shipping_Cost'],
    ...
    function checkout_cart_details_old()
	{
		if($this->session->userdata('cust_logged_in'))
		{	
			$session_data = $this->session->userdata('cust_logged_in');
    ...
    public function checkout2()
	{
		...
		if($this->session->userdata('cust_logged_in'))
		{
			...
			/**********AMIT KAMBLE 11-09-2019********************************/
				...
				$data['payment_auth_key'] = $Enroll_details->Mpesa_auth_key;
                /*********************************************************/
...
//********** mpesa trans_id confirmation 05-10-2019 ***********************
				...
				//********** mpesa trans_id confirmation 05-10-2019 ***********************
					...
					if ($cart = $this->cart->contents())
					{
						...
						//		}
							...
							//	echo "req RequiredCondiments---"; print_r($item["options"]["RequiredCondiments"]);		
								...
								{
								/**********Get Seller Details**********/
									$Seller_result = $this->Shopping_model->Get_Seller_details($Brand_Enrollement_id,$Company_id);	
										...
										$lp_details = $this->Shopping_model->get_loyalty_program_details($Company_id,$Brand_Enrollement_id,$prog,$lv_date_time);
											...
											{	
												...
												// $transaction_amt = $grand_total;
												 ...
												 // echo "---------grand_total-------".$grand_total; exit;
												$grand_total1=0;
							...
							// echo "<br><b>Weighted_redeem_points--------- </b>".$Weighted_redeem_points; exit;	
								...
								//***** sandeep ******** 24-03-2020 ****************
									...
									if($_REQUEST['Redeemed_discount_voucher'] != "")
									{
										...
										$redeemed_VoucherDiscountAmt = $_REQUEST['redeem_voucher_amt'];
									}
								//***** sandeep ******** 24-03-2020 ****************
								//******** 18-03-2020****sandeep**********	
					...
					// }
						...
						/************New Change By Ravi**30-10-2017***************/
							...
							$result4 = $this->Shopping_model->update_billno($Brand_Enrollement_id,$billno_withyear);
						/*********** Update Current Balance ***************/
			...
			//************* online order api call 28-08-2019 sandeep *********************
				if($delivery_outlet_details->Seller_api_url != "" || $delivery_outlet_details->Seller_api_url != NULL)
					...
					"Discount_voucher_codeused" =>$_REQUEST['Redeemed_discount_voucher'], "Discount_voucher_amountused" =>number_format($_REQUEST['VoucherDiscountAmt'],2),
					...
					//		print_r($input_args3);
						...
						// echo "DiscountAmt----------".$_SESSION['DiscountAmt']; 
						// echo "-----------VoucherDiscountAmt----------".$_REQUEST['redeem_voucher_amt']; exit;
						if($result == null || $result == "" || $response['Status'] != "0000")
						{
							...
							$Email_content = array
								(
									...
									'VoucherDiscountAmt' => $_REQUEST['redeem_voucher_amt'],
									'mpesa_BillAmount' => $mpesa_BillAmount,  
...
/************************01-10-2019 Nilesh cahnge address***************************/
		...
		$day_of_week = date('N', strtotime($Current_day));
			...
			//******** 03-10-2019 sandeep*********************						
				...
				// echo "Seller_api_url --".$Seller_api_url."<br>";		
						...
						if(curl_errno($ch))
						{
							// throw new Exception(curl_error($ch));
							 $Outlet_status = 2;
							 $inif = "in curl_errno if";
						} 
						else 
						{
							$inif = "in curl_errno else";
							$response = json_decode($result, true);
							if($result == null || $result == "" || $response['Status'] != "0000")
							{
								$Outlet_status = 2;
				...
				//		curl_setopt($ch, CURLOPT_POSTFIELDS,$input_args);
						...
						{
							// $inif = "in curl_err else";
							//	  echo $result;
						}
						
						$response = json_decode($result, true); // for demo
						// $response1 = json_decode(json_decode($result, true),true); // for live
	...
	//******** 03-10-2019 sandeep **********************
		...
		// print_r($result);
		// echo "echo-----".$inif.'***'.$response;
		// echo "----result-----".$result;
		// echo "----response-----".$response['Status'];
		// echo "----response1-----".$response1['Status']."---------";
		$this->output->set_content_type('application/json');
	...
	public function CreateToken()
	{
		if($this->session->userdata('cust_logged_in'))
		{
			...
			$RedirectURL = base_url().'index.php/Shopping/checkout_cart_details';
			$BackURL = base_url().'index.php/Shopping/checkout_cart_details';
