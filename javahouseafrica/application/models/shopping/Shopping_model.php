...
{
    public function get_all_items($limit,$start,$Company_id,$Merchandize_category_id,$Sort_by,$Sort_by_merchant,$Sort_by_brand,$Sort_by_gender,$Sort_by_name,$Selected_outlet)
	{
		$Todays_date=date("Y-m-d");
		$this->db->from('igain_company_merchandise_catalogue as A');
		$this->db->join('igain_pos_item_linked_outlets as B', 'B.Merchandize_item_code = A.Company_merchandize_item_code AND A.Cost_price !=0');
		
		if($Merchandize_category_id==0)//ALL
		{
			 $this->db->where(array('A.Company_id'=>$Company_id,'A.Valid_from <='=>$Todays_date,'A.Valid_till >='=>$Todays_date,'A.show_item'=>1,'A.Active_flag'=>1,'A.Ecommerce_flag'=>1,'A.Link_to_Member_Enrollment_flag'=> 0,'A.Send_once_year'=> 0,'A.Send_other_benefits'=> 0,'B.Outlet_id' =>$Selected_outlet));	
		}
		else
		{
			$this->db->where(array('A.Company_id'=>$Company_id,'A.Merchandize_category_id'=>$Merchandize_category_id,'A.Valid_from <='=>$Todays_date,'A.Valid_till >='=>$Todays_date,'A.show_item'=>1,'A.Active_flag'=>1,'A.Ecommerce_flag'=>1,'A.Link_to_Member_Enrollment_flag'=> 0,'A.Send_once_year'=> 0,'A.Send_other_benefits'=> 0,'B.Outlet_id' =>$Selected_outlet));
		}
	...
	public function Get_merchandize_item12($item_code,$Company_id)
	{
		  ...
		  $this->db->from('igain_company_merchandise_catalogue');
		  $this->db->where(array('Company_merchandize_item_code' => $item_code, 'Company_id' => $Company_id,'Active_flag'=>1));
		  $query11 = $this->db->get();
	...
	function Get_Merchandize_ecommerce_Items_Count($Company_id,$Selected_outlet)
	{
		$Todays_date=date("Y-m-d");
		$this->db->select('Company_merchandise_item_id');
		$this->db->from('igain_company_merchandise_catalogue as A');
		$this->db->join('igain_pos_item_linked_outlets as B', 'A.Company_merchandize_item_code = B.Merchandize_item_code AND B.Outlet_id = "'.$Selected_outlet.'"');
		$this->db->where(array('A.Ecommerce_flag'=>1,'A.Active_flag'=>1,'A.show_item'=>1,'A.Company_id'=>$Company_id,'A.Valid_from <='=>$Todays_date,'A.Valid_till >='=>$Todays_date));
		$sql=$this->db->get();		
		return $sql->num_rows();
	}
	/**************Ravi---22-09-2017***************/
	...
	public function get_req_optional_condiment_que($optGroup_code,$optional_condimentType,$Company_id)
	{
		...
		$this->db->where(array("Group_code" => $optGroup_code,"Condiment_type" => $optional_condimentType,"Company_id" => $Company_id,"Active_flag" => 1));
		$this->db->limit(1,0);
	...
	public function get_main_item($item_code,$company_id)
	{
		...
		$this->db->where(array("A.Menu_Item_code" => $item_code,"A.Item_flag" => "MAIN","A.Company_id" => $company_id, "B.Company_id" => $company_id,"B.Active_flag" =>1));
	...
	//******** sandeep POS ****************************
//***********Nilesh start 30-04-2020******************/	
	function FetchOutletdetails($Company_id)
	{
		$this->db->select("*");
		$this->db->from('igain_enrollment_master');
	...
	function Get_Outlet_Menu_Category($Company_id,$Selected_outlet)
	{
		$Todays_date=date("Y-m-d");
		$this->db->select('A.Merchandize_category_id,A.Merchandize_category_name');
		$this->db->from('igain_merchandize_category AS A');
		$this->db->join('igain_company_merchandise_catalogue as C','A.Merchandize_category_id=C.Merchandize_category_id AND C.Cost_price !=0');
		$this->db->join('igain_pos_item_linked_outlets as B', 'B.Merchandize_item_code = C.Company_merchandize_item_code AND B.Outlet_id ="'.$Selected_outlet.'"');
		$this->db->where(array('A.Active_flag'=>1,'A.Company_id'=>$Company_id));
		$this->db->where(array('C.Company_id'=>$Company_id,'C.Valid_from <='=>$Todays_date,'C.Valid_till >='=>$Todays_date,'C.show_item'=>1,'C.Active_flag'=>1,'C.Ecommerce_flag'=>1,'C.Link_to_Member_Enrollment_flag'=> 0,'C.Send_once_year'=> 0,'C.Send_other_benefits'=> 0));
		$this->db->where_not_in('A.Merchandize_category_id', array('10','15'));
