...
if($Order_details!=NULL)
{
	foreach($Order_details as $Order_det)
	{
		...
		$Seller_name=$Order_det["Seller_name"];
		$Delivery_method=$Order_det["Delivery_method"];
		$Table_no=$Order_det["Table_no"];
	} ?>
	<h1 style="float:left;">Order No. : <?php echo $Bill_no; ?></h1>	&nbsp; &nbsp; 
	<button type="button" class="btn btn-primary" style="text-transform: none;" onclick="Reorder('<?php echo $Bill_no; ?>','<?php echo $Seller; ?>','<?php echo $Seller_name; ?>','<?php echo $Delivery_method; ?>','<?php echo $Table_no; ?>');">REORDER ALL ITEMS</button>
<?php }
?>
</section>
				...
				//********* sandeep ***************
								?>
									<tr>
										...
										<td>   
										  <button type="button" class="btn btn-primary" style="text-transform: none;" onclick="add_to_cart1('<?php echo $Order_det['Company_merchandise_item_id']; ?>','<?php echo $Order_det["Item_code"]; ?>','<?php echo preg_replace('/[^a-zA-Z0-9_ %\[\]\.\(\)%&-]/s','',$Order_det['Merchandize_item_name']); ?>','<?php echo $Order_det['Billing_price']; ?>',29,'<?php echo $Order_det['Merchandize_Partner_branch']; ?>','<?php echo $Order_det['Company_merchandise_item_id']; ?>','<?php echo $Order_det['Item_size']; ?>','<?php echo $Order_det['Item_Weight']; ?>','<?php echo $Order_det['Weight_unit_id']; ?>','<?php echo $Order_det['Merchandize_Partner_id']; ?>','<?php echo $Order_det['Partner_state']; ?>','<?php echo $Order_det['Partner_Country']; ?>','<?php echo $Order_det['Seller_id']; ?>','<?php echo $Order_det['Merchant_flag']; ?>','<?php echo $Order_det['Cost_price']; ?>','<?php echo $Order_det['VAT']; ?>','<?php echo $Order_det['Merchandize_category_id']; ?>','<?php echo $Order_det['remark2']; ?>','<?php echo $Order_det['remark3']; ?>','<?php echo $Order_det["Bill_no"]; ?>','<?php echo $Order_det['Combo_meal_flag']; ?>','<?php echo $Order_det['Seller']; ?>','<?php echo $Order_det['Seller_name']; ?>','<?php echo $Order_det['Delivery_method']; ?>','<?php echo $Order_det['Table_no']; ?>');">REORDER</button>
										</td>
								...
								<tr>
									...
									<td colspan="3">&nbsp;
										...
										<div class="form-group" id="outlet_status" style="display:none;">
											<font color="red">No Orders Can be Placed at this Outlet !!!</font>
										</div>
									</td>
								</tr>
						</tbody>
						<tfoot>
							...
							<tr>
								...
								<td colspan="2"><?php echo $Symbol_of_currency." ".number_format((float)$MpesaPaidAmount1, 2);?>&nbsp;<?php if($MpesaPaidAmount1 !=0){ echo '<span style="font-size: 12px;">('.$Mpesa_TransID.')</span>'; } ?></td>
							</tr>
							<!--<tr>
	...
	$.ajax(
    {
        ...
        success: function(data)
        {
		   ...
		   {  
			   $("#outlet_status").show();
			   $("#outlet_status2").show();
			   $("#outlet_status2").html('<font color="red">'+Seller_name+' is Currently Closed</font>');
			   return false;
		   }
		   else if(data==2)//close
		   {
			   $("#outlet_status2").show();
			   $("#outlet_status2").html('<font color="red">'+Seller_name+' is Currently not processing online Orders</font>');
			   return false;
		   }
		   else
		   {  
...
function Reorder(Bill_no,Seller,Seller_name,Delivery_method,Table_no)
{
	// show_loader();
