...
class Shopping_model extends CI_Model 
{
	...
	public function get_offer_selected_items($Offer_code,$Company_id)
	{
		...
		$sqlqury = $this->db->get();
		// echo "<br><br>get_offer_selected_items :: ".$this->db->last_query();
	...
	public function Get_member_sent_vouchers($Company_id,$enroll_id,$Company_merchandize_item_code)
	{
		$this->db->select('Voucher_code,Quantity,Cost_price');
		$this->db->from('igain_company_send_voucher as v');
		$this->db->join('igain_giftcard_tbl as g','v.Company_id=g.Company_id AND v.Voucher_code=g.Gift_card_id');
		$this->db->where(array('v.Company_id'=>$Company_id, 'v.Enrollement_id'=>$enroll_id, 'v.Company_merchandize_item_code'=>$Company_merchandize_item_code, 'g.Card_balance >'=> '0','g.Payment_Type_id' => '997','v.Voucher_id' =>'0','v.Offer_code !=' => ''));
		$sql77 = $this->db->get();
		echo "<br><br>Get_member_sent_vouchers :: ".$this->db->last_query();//die;
		if($sql77->num_rows() > 0)
		{
			foreach ($sql77->result() as $row)
			{
				$data[] = $row;
			}
			return $data;
	...
	function Get_stamp_vouchers_details($Card_id,$Company_id,$Voucher_code,$Vouchers_min_price)
	{
		$today = date("Y-m-d");
		
		$this->db->select("igain_giftcard_tbl.Gift_card_id,igain_giftcard_tbl.Card_balance,igain_giftcard_tbl.Valid_till,igain_giftcard_tbl.Discount_percentage,B.Company_merchandize_item_code,B.Quantity,(B.Cost_price) as Reduce_product_amt,Offer_description,Offer_name");
		...
		$this->db->where(array('igain_giftcard_tbl.Card_id' => $Card_id,'igain_giftcard_tbl.Company_id' => $Company_id,'igain_giftcard_tbl.Payment_Type_id' => '997','B.Voucher_code' => $Voucher_code,'B.Cost_price' => $Vouchers_min_price,'B.Voucher_id' =>0,'B.Offer_code !=' => ''));			
		
		$this->db->where(array('igain_giftcard_tbl.Card_balance >' => '0','igain_giftcard_tbl.Valid_till >=' => $today));
		$this->db->group_by('B.Voucher_code');
		$this->db->order_by('igain_giftcard_tbl.Create_date','DESC');
		$query = $this->db->get('igain_giftcard_tbl');			
		 // echo "<br<br><br<br>Get_stamp_vouchers_details   ".$this->db->last_query()."---<br>"; 
		if($query -> num_rows() > 0)
		{
			return $query->result_array();
		}
	...
	function Get_lowest_sent_vouchers($Card_id,$Company_id)
	{
		$today = date("Y-m-d");
		...
		$this->db->where(array('igain_giftcard_tbl.Card_id' => $Card_id,'igain_giftcard_tbl.Company_id' => $Company_id,'igain_giftcard_tbl.Payment_Type_id' => '997','B.Voucher_id' =>0,'B.Offer_code !=' => ''));			
	...
	public function update_transaction_balance($data,$cid,$Enrollement_id,$Bill_no,$company_id)
	{
		//$this->db->set('point_balance'); 
		$this->db->where('Enrollement_id', $Enrollement_id,'Bill_no', $Bill_no, 'Card_id', $cid, 'Company_id', $company_id); 
		$this->db->update('igain_transaction', $data);
		return true;
	}
