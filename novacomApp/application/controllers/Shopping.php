...
class Shopping extends CI_Controller 
{
	...
	{ 
		...
		{	
			...
			*************** Revi Product Voucher logic 01-07-2020 **************************/
			...
			$Product_vouchers_withoutStap_array=array();
			$Product_vouchers_withoutStap11=array();
			$Vouchers_list =array();
			$Vouchers_min_price =array();
			$ItemCodeArr =array();
			if ($cart = $this->cart->contents())
			{
				...
				/********************************Ravi 02-07-2020************************************/		
				/*****************AMIT KAMBLE 19-04-2021 STAMP REDEEM VOUCHER****/
				$ItemCodeArr[$ItemCode]=$item['qty'];
				/*
					$Query1= $this->Shopping_model->Get_member_sent_vouchers($data['Company_id'],$data['enroll'],$item["options"]['Company_merchandize_item_code']);
					...
					if($Query1 != NULL)
					{
						...
						{
							//30001,30002
							// echo "Voucher_code ".$rec1->Voucher_code; 8368473630,8368473630
							$temp[$rec1->Voucher_code][] = $item['qty']; //1,2
							
							if($item['qty'] < $rec1->Quantity)
							{
								$tempCost_price[$rec1->Voucher_code][] = ($item['qty']*$item['price']); //
								$Min_tempCost_price=array_sum($tempCost_price[$rec1->Voucher_code]);//660
							}
							else
							{
								$tempCost_price[$rec1->Voucher_code][] = $rec1->Cost_price; //900,660
								$Min_tempCost_price=min($tempCost_price[$rec1->Voucher_code]);//660
							}
							$Sum_item_cart_qty=array_sum($temp[$rec1->Voucher_code]);//3
							
							// echo "<br> Quantity ".$rec1->Quantity;
							// echo "<br> Cost_price ".$rec1->Cost_price;
							// echo "<br> Sum_item_cart_qty ".$Sum_item_cart_qty;
							
							if($Sum_item_cart_qty >= $rec1->Quantity)//1 >= 3,3>=3
							{
								// echo "<br>Voucher_code ".$rec1->Voucher_code;
								$Vouchers_list[]= $rec1->Voucher_code;//8368473630
								
								//SELECT Voucher_code,Company_merchandize_item_code,Quantity,(Cost_price) as Min_cost FROM `igain_company_send_voucher` WHERE `Company_id`=8 and `Voucher_code`='8368473630' order by Cost_price asc limit 1
								//30002,3,660
								if($rec2->Company_merchandize_item_code==$item["options"]['Company_merchandize_item_code'] && $item['qty']<$rec2->Quantity)
								{
									$Vouchers_min_price[$rec1->Voucher_code]= $item['price'] * $item['qty'];//440
									$Rem_qty = ($rec2->Quantity-$item['qty']);//3-2=1
									// $Min_tempCost_price=($Rem_qty*);//
								}
								
								$Vouchers_min_price[$rec1->Voucher_code]= $Min_tempCost_price;//660
								// echo "<br> Min_tempCost_price ".$Min_tempCost_price;
							}
							
							
						}
					}*/
					// print_r($tempCost_price);
				...
				/********************************Ravi 02-07-2020************************************/
					...
					/****************Ravi 15-02-2021 Product Voucher without stamp***********************/
					$Product_vouchers_withoutStap= $this->Igain_model->get_member_product_vouchers($data['Card_id'],$data['Company_id'],$item["options"]['Company_merchandize_item_code'],$item['qty']);
					if($Product_vouchers_withoutStap){
						
						// echo"---ItemCode---".$ItemCode."-------Item_price---".$Item_price."--Card Balance--".$Product_vouchers_withoutStap[2]["Card_balance"]."-<br>";
						
						foreach($Product_vouchers_withoutStap as $prodWS){
							
							
							$Cardbalance=$prodWS["Card_balance"];
							if($prodWS["Card_balance"] > $Item_price){
								 
								$prodWS["Card_balance"]=$Item_price;
								
							}
							$Product_vouchers_withoutStap11[] = array(
								
								'Gift_card_id'=>$prodWS["Gift_card_id"],								
								'Card_balance'=>number_format($prodWS["Card_balance"],2),
								'Cardbalance'=>number_format($Cardbalance,2),
								'Valid_till'=>$prodWS["Valid_till"],
								'Discount_percentage'=>$prodWS["Discount_percentage"],
								'Company_merchandize_item_code'=>$prodWS["Company_merchandize_item_code"],
								'Merchandize_item_name'=>$prodWS["Merchandize_item_name"],
								'Quantity'=>$prodWS["Quantity"],
								'Reduce_product_amt'=>$prodWS["Reduce_product_amt"]
							
							);							
						}
												
						$Product_vouchers_withoutStap_array[] =$Product_vouchers_withoutStap11;
					}
					unset($Product_vouchers_withoutStap11);
					/****************Ravi 15-02-2021 Product Voucher without stamp***********************/
				...
				/****************************AMIT***************************************************************/
				// print_r($ItemCodeArr);
				$Get_lowest_sent_vouchers= $this->Shopping_model->Get_lowest_sent_vouchers($data['Card_id'],$data['Company_id']);
						...
						if(array_key_exists($rec1->Company_merchandize_item_code,$ItemCodeArr))
						{
							...
							if($RemQTY!=0)//
							{
								if($Cart_item_QTY >= $RemQTY )
								{
									...
									if($lowest_flag!=0){$data['Free_item_arr'][$rec1->Company_merchandize_item_code] = $RemQTY;}
									...
									// echo "<br>Reduce_product_amt".$Reduce_product_amt;
									$lowest_flag=0;
							...
							if($Cart_item_QTY < $rec1->Voucher_Qty && $RemQTY==0)//
							{
								...
								// echo "<br>lv_Voucher_code ".$lv_Voucher_code;
							}
				...
				// echo "<br><br>Unique_Vouchers_list--";print_r($data['Unique_Vouchers_list']);
				// echo "<br><br>Vouchers_price--";print_r($data['Vouchers_price']);
				// echo "<br><br>Free_item_arr--";print_r($data['Free_item_arr']);
				
				/* $Unique_Vouchers_list = array_unique($Vouchers_list);
				for($k=0;$k<count($Unique_Vouchers_list);$k++)
				{
					$ProductInPercentageVouchers= $this->Shopping_model->Get_stamp_vouchers_details($data['Card_id'],$data['Company_id'],$Unique_Vouchers_list[$k],$Vouchers_min_price[$Unique_Vouchers_list[$k]]);
					$Product_vouchers_array[] =$ProductInPercentageVouchers;
				} */
				...
				// echo "<br><br>--";
					// print_r($Vouchers_list);echo "---<br><br>";
					// print_r($Unique_Vouchers_list);echo "<br><br>";
				// print_r($temp);
				// echo "<br>".array_sum($temp['9743048774']);
				/****************Ravi 15-02-2021 Product Voucher without stamp***********************/
				// var_dump($Product_vouchers_withoutStap_array);
				$data['ProductVouchersWithoutStamp'] = $Product_vouchers_withoutStap_array;
				/****************Ravi 15-02-2021 Product Voucher without stamp***********************/
				...
				$data['ProductInPercentageVouchers'] = $Product_vouchers_array;
					// echo "<br><br>";
					// print_r($Product_vouchers_array);echo "<br><br>";
    ...
    public function CheckoutPayment()
	{
		...
		/**********************Nilesh VerifyToken 09-07-2020****************************/	
			...
			/* Wallet_balance 17-02-2022  */
				if($_REQUEST["Wallet_balance"] && $_REQUEST["wallectBalance_amt"]){
					$wallectBalance_amt=$_REQUEST["wallectBalance_amt"];
					$Wallet_balance=$_REQUEST["Wallet_balance"];
				} else {
					$wallectBalance_amt=0;
					$Wallet_balance=0;
				}
			/* Wallet_balance 17-02-2022 */
		...
		//************** sandeep discount voucher 30-01-2020 ********************************
			...
			{ 				
						/***************Apply Loyalty Rule****************/
							...
							$Cust_Wallet_balance = $data["Enroll_details"]->Wallet_balance;
 ...
 */											$Extra_earn_points = $result->Extra_earn_points;
									...
									//****************** 28-08-2019 sandeep ***********************************			
											...
											/* $wallectBalance_amt=$_REQUEST["wallectBalance_amt"];
											$Wallet_balance=$_REQUEST["Wallet_balance"]; */
											$Weighted_wallectBalance_amt = round(($wallectBalance_amt * $item_total_amount) / $subtotal);
	...
	/*echo "<br><b>PaidAmount--------- </b>".$PaidAmount;	*/									
										...
										/* $wallectBalance_amt=$_REQUEST["wallectBalance_amt"];
											$Wallet_balance=$_REQUEST["Wallet_balance"]; */
											$Weighted_wallectBalance_amt = round(($wallectBalance_amt * $item_total_amount) / $subtotal);
											if($Weighted_wallectBalance_amt > 0){
												$Weighted_wallectBalance_amt=$Weighted_wallectBalance_amt;
											}else {
												$Weighted_wallectBalance_amt=0;
											}
											...
											$data123 = array(
													...
													'Wallet_amount' => $Weighted_wallectBalance_amt,
												...
												// echo "----sql Insert-Transaction_detail--".$this->db->last_query()."----<br>";
						...
						// echo "Redeemed_discount_voucher ".$_REQUEST['Redeemed_discount_voucher'];
						if($_REQUEST['Redeemed_discount_voucher'] != "0")
						{
							// echo $Discount_voucher= "";
							$redeemed_discount_voucher = $_REQUEST['Redeemed_discount_voucher'];
							// echo "Redeemed_discount_voucher--".$redeemed_discount_voucher;
							...
							/* $Free_quantity = $this->Shopping_model->Get_freeVoucher_quantity($data['Company_id'],$redeemed_discount_voucher);
							...
							$Free_item_qty=$_REQUEST["Free_item_$Company_merchandize_item_code"];
							$updateData1 = array('Free_item_quantity' => $Free_item_qty);
							$this->db->where(array('Trans_id' => $Transaction_detail,'Item_code' => $Company_merchandize_item_code));
						...
						if($item['Stamp_item_flag'] == 1)
						{
							...
							if($Product_offers != null)
							{
								...
								{	
									...
									$Offers_items = $this->Shopping_model->get_offer_selected_items($offer['Offer_code'],$data['Company_id']);
									...
									if($Offers_items != NULL)
									{
										foreach($Offers_items as $rec)
										{
											...
											$Total_item_purchase[] = $this->Shopping_model->get_item_purchase_count($rec->Company_merchandize_item_code,$data['Company_id'],$data['enroll'],$offer['From_date'],$offer['Till_date']);
										}
									}	
										$Total_count_item= array_sum($Total_item_purchase);	
									
										// echo "<br><br>Total_count_item :: ".$Total_count_item;	
										// echo "<br><br>Buy_item :: ".$offer['Buy_item'];	
										// echo "<br><br>Total_count_item($Total_count_item) >= Buy_item(".$offer['Buy_item'].")";	
										
									if($Total_count_item >= $offer['Buy_item'])
									{
										$Total_sent_voucher = $this->Shopping_model->member_sent_offers($data['Company_id'],$data['enroll'],$offer['Offer_code'],$offer['Free_item_id']);
										...
										for($A=1;$A<= $Need_to_Send_Voucher;$A++)
										{
											...
											// echo "<br><br>A::$A";
											$FreeItem = $this->Shopping_model->get_offer_free_items($offer['Offer_code'],$data['Company_id']);
			...
			//*****************************AMIT KAMBLE STAMP END**********************************************		
						...
						/************************* Update Current Balance ************************************/
							...
							/*********************New Change By Ravi**30-10-2017************************/
								...
								$Update_Wallet_balance = $Cust_Wallet_balance - $Wallet_balance;
								
								
								
								$up = array('Current_balance' => $Update_Current_balance, 'total_purchase' => $Update_total_purchase, 'Total_reddems' => $Update_total_reddems, 'Wallet_balance' => $Update_Wallet_balance);
								...
								/* Update Available Balance 17-02-2022 */
								
									$Avaldata = array('Available_balance' => $Update_Current_balance, 'Available_wallet_balance' => $Update_Wallet_balance);
									
									$resultAvl = $this->Shopping_model->update_transaction_balance($Avaldata,$cid,$cust_enrollment_id,$bill,$Company_id);
									// echo"<br>--resultAvl-----".$this->db->last_query();
						...
						if($result == null || $result == "" || $response['Status'] != "0000")
						{
								...
								(
                                       'Notification_type' => 'Thank you for your Purchase at '. $delivery_outlet_details->First_name.' - '.$delivery_outlet_details->Last_name,
										...
										'mpesa_BillAmount' => $mpesa_BillAmount,  
										'wallectBalance_amt' => $wallectBalance_amt,  
...
/************************************************Akshay Start***********************************************/
	...
	{
		$input_args = Array ('TransID' => $newTransID ,'BalanceDue' => $Final_Grand_total,"BillRefNumber" =>$BillRefNumber,"PaymentMethod"=> "MOBILEMONEY","PaymentType"=> "MPESA");
				
		$input_args = json_encode($input_args);
		
		// $MpesaPushurl = "https://apps.novacom.co.ke:7074/api_b2b/mpesastkpush";
		// $MpesaPushurl = "https://zdpp.online/api_b2b/mpesastkpush";
	...
	public function Verify_mpesa()
	{
		...
		{	
			...
			{				
				...
				/**************************************************************************/
					$Mpesa_step2_Data = array (
						'Company_id' => $data['Company_id'],
						'Membership_id' => $data['Card_id'],
						'Bill_no' => $BillRefNumber,
						'Result_code' => $response['ResultCode'],
						'Result_desc' => $response['ResultDesc'],
						'Balance_due' => $response['BalanceDue'],
						'Paid_amount' => $response['PaidAmount'],
						'Trans_id' => $response['TransID'],
						'Json_input' => $input_args,
						'Json_output' => $result
					);	
					
					$Insert_MpesaData = $this->Shopping_model->insert_step2_validation_log($Mpesa_step2_Data);
							
				/**************************************************************************/
					if($response['BalanceDue'] == $response['PaidAmount'])
					{
						$this->output->set_content_type('application/json');
					...
					else if($response['BalanceDue'] == $response['PaidAmount'] || $i==7)
					{
						$this->output->set_content_type('application/json');
						$this->output->set_output(json_encode(array('response' => $result)));	
