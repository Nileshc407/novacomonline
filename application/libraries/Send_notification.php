...
/* PHPMailer Include files */
  ...
  class Send_notification {
    ...
    public function send_Notification_email($Enrollment_id, $Email_content, $Seller_id, $Company_id) {
      ...
      $Wallet_balance = $customer_details->Wallet_balance;
      ...
      $company_smtp_details = $this->CI->Igain_model->get_company_smtp_details($Company_id);
	  if($company_smtp_details!= Null)
	  {
		  $smtp_protocol = $company_smtp_details->smtp_protocol;	
		  $smtp_host = $company_smtp_details->smtp_host;	
		  $smtp_user = $company_smtp_details->smtp_user;	
		  $smtp_pass = $company_smtp_details->smtp_pass;	
		  $smtp_port = $company_smtp_details->smtp_port;	
	  }
	  else
	  {
			$parent_company_smtp_details = $this->CI->Igain_model->get_company_smtp_details(3);
			
			$smtp_protocol = $parent_company_smtp_details->smtp_protocol;	
			$smtp_host = $parent_company_smtp_details->smtp_host;	
			$smtp_user = $parent_company_smtp_details->smtp_user;	
			$smtp_pass = $parent_company_smtp_details->smtp_pass;	
			$smtp_port = $parent_company_smtp_details->smtp_port;			
	  }
      ...
      if ($Template_type == "Communication") {
        ...
        $subject1 = strip_tags($Offer_description);
        ...
        $search_variables = array('$First_name','$Customer_name', '$Offer_description', '$Start_date', '$End_date', '$Member_website', '$Company_name','$Logo','$Merchant_name'); 
        $inserts_contents = array($First_name,$Customer_name, $Offer_description, $Start_date, $End_date, $Member_website, $Company_name,$Logo,$Merchant_name);
      ...
      if ($Template_type == "Pos_json_error") {
		  
		$pos_html = $Email_content['pos_html'];
		$Pos_error_file = $Email_content['Pos_error_file'];
		$Notification_send_to_email = 1;
		$User_email_id = 'loyaltyapp@novacom.co.ke';
        ...
        $subject = "$Server_name PoS API Error file for ".$company_details->Company_name;
      ...
      if ($Template_type == "Issue_bonus") {
        ...
        $subject = "Topup from " . $company_details->Company_name;
		...
		$First_name = ucwords($customer_details->First_name);
        /*         * **********************************Variable Replace Code***************************** */
        $search_variables = array('$First_name','$Customer_name', '$Merchant_name', '$Membership_ID', '$topup_amt', '$Current_balance', '$Transaction_date', '$Member_website', '$Company_name', '$Gooogle_Play', '$iOs_app_store', '$Comapany_Currency','$Symbol_currency','$Remarks','$Logo'); //
        $inserts_contents = array($First_name,$Customer_name, $Merchant_name, $customer_details->Card_id, round($Email_content['topup_amt']), $Current_point_balance, $Transaction_date, $Member_website, $Company_name, $Gooogle_Play, $iOs_app_store, $Comapany_Currency, $Email_content['Symbol_currency'],$Email_content['Remarks'],$Logo);
        $html = str_replace($search_variables, $inserts_contents, $body);
        /*         * **********************************Variable Replace Code***************************** */
      }
	  if ($Template_type == "Add_wallet_balance") 
	  {
        $Communication_id = 0;
        $Offer = $Email_content['Notification_type'];
        $Transaction_date = date("d M Y", strtotime($Email_content['Todays_date']));
        $subject = "Wallet Credit Transaction at ". $company_details->Company_name;
		...
		include "./$App_folder_name/template/Add_wallet_balance.php";
		...
		$First_name = ucwords($customer_details->First_name);
        /*         * **********************************Variable Replace Code***************************** */
        $search_variables = array('$First_name','$Customer_name', '$Merchant_name', '$Membership_ID', '$topup_amt', '$Current_balance','$Wallet_balance', '$Transaction_date', '$Member_website', '$Company_name', '$Gooogle_Play', '$iOs_app_store', '$Comapany_Currency','$Symbol_currency','$Remarks'); //
        $inserts_contents = array($First_name,$Customer_name, $Merchant_name, $customer_details->Card_id, $Email_content['topup_amt'], $Current_point_balance,$Wallet_balance, $Transaction_date, $Member_website, $Company_name, $Gooogle_Play, $iOs_app_store, $Comapany_Currency, $Email_content['Symbol_currency'], $Email_content['Remarks']);
        $html = str_replace($search_variables, $inserts_contents, $body);
        /*         * **********************************Variable Replace Code***************************** */
      }
      if ($Template_type == "Redeem") {
        ...
        $Offer = $Email_content['Notification_type'];
        $Transaction_date = date("d M Y", strtotime($Email_content['Todays_date']));
        $subject = "Adjustment/Debit Transaction at " . $company_details->Company_name;

        ob_start();
        // include './application/libraries/Email_templates/Just_Redeem.php';
		
		include "./$App_folder_name/template/Debit_balance.php";
        ...
        /*         * **********************************Variable Replace Code***************************** */
        $search_variables = array('$First_name','$Customer_name', '$Merchant_name', '$Membership_ID', '$Redeem_points', '$Current_balance', '$Transaction_date', '$Member_website', '$Company_name', '$Gooogle_Play', '$iOs_app_store', '$Comapany_Currency', '$Remarks', '$Logo'); //
        $inserts_contents = array($First_name,$Customer_name, $Merchant_name, $customer_details->Card_id, round($Email_content['Redeem_points']), $Current_point_balance, $Transaction_date, $Member_website, $Company_name, $Gooogle_Play, $iOs_app_store, $Comapany_Currency,$Email_content['Remarks'],$Logo);
        $html = str_replace($search_variables, $inserts_contents, $body);
        /*         * **********************************Variable Replace Code***************************** */
      }
	  if ($Template_type == "Wallet_debit") 
	  {
        $Communication_id = 0;
        $Offer = $Email_content['Notification_type'];
        $Symbol_currency = $Email_content['Symbol_currency'];
        $Transaction_date = date("d M Y", strtotime($Email_content['Todays_date']));
        $subject = "Wallet Debit Transaction at " . $company_details->Company_name;

        ob_start();
        
		include "./$App_folder_name/template/Wallet_debit_balance.php";
        ...
        /*         * **********************************Variable Replace Code***************************** */
        $search_variables = array('$First_name','$Customer_name', '$Merchant_name', '$Membership_ID', '$Debit_amount', '$Current_balance', '$Transaction_date', '$Member_website', '$Company_name', '$Gooogle_Play', '$iOs_app_store', '$Comapany_Currency','$Wallet_balance','$Symbol_currency','$Remarks','$Logo'); //
        $inserts_contents = array($First_name,$Customer_name, $Merchant_name, $customer_details->Card_id,$Email_content['Debit_amount'], $Current_point_balance, $Transaction_date, $Member_website, $Company_name, $Gooogle_Play, $iOs_app_store, $Comapany_Currency,$Wallet_balance,$Symbol_currency,$Email_content['Remarks'],$Logo);
        $html = str_replace($search_variables, $inserts_contents, $body);
        /*         * **********************************Variable Replace Code***************************** */
      }
	...
	{  
		...
		if(count($Voucher_array) > 0)
		{
			...
			$vouchers = $Voucher_array['voucher_no'];
			$amount = $Voucher_array['amount'];
		}
		else
		{
		...
		/************************************Variable Replace Code******************************/
        $search_variables = array('$First_name','$Logo', '$Customer_name', '$Transaction_date', '$Order_type', '$delivery_outlet', '$Orderno', '$POS_bill_no', '$Symbol_of_currency', '$subtotal', '$Company_Currency', '$total_loyalty_points', '$grand_total', '$discountVal', '$Cust_wish_redeem_point', '$EquiRedeem', '$Current_point_balance', '$vouchers', '$VoucherDiscountAmt','$amount'); // 
		
        $inserts_contents = array($First_name,$Logo,$Customer_name, $Transaction_date, $Order_type, $delivery_outlet, $Orderno, $POS_bill_no, $Symbol_of_currency, $subtotal, $Company_Currency, $total_loyalty_points, $grand_total, $discountVal, $Cust_wish_redeem_point, $EquiRedeem, $Current_point_balance, $vouchers,$VoucherDiscountAmt,$amount);
        $html = str_replace($search_variables, $inserts_contents, $body);
        /************************************Variable Replace Code******************************/
	}
    ...
    if ($Template_type == "Purchase_cancel") 
	{
      ...
      if ($Template_type == "Enroll") {
        ...
        /*         * **********************************Variable Replace Code***************************** */
        $search_variables = array('$First_name','$Customer_name', '$Merchant_name', '$Membership_ID', '$Enrolled_under', '$User_email_id', '$Phone_no', '$User_pwd','$Pwdlink', '$pinno', '$Member_website', '$Company_name', '$Gooogle_Play', '$iOs_app_store','$Logo','$Cust_apk_link','$Cust_ios_link'); //
        $inserts_contents = array($First_name,$Customer_name, $Merchant_name, $Membership_ID, $Company_name, $User_email_id, $Phone_no, $User_pwd,$PwddataLink, $pinno, $Member_website, $Company_name, $Gooogle_Play, $iOs_app_store,$Logo,$Cust_apk_link,$Cust_ios_link);
        $html = str_replace($search_variables, $inserts_contents, $body);
        /*         * **********************************Variable Replace Code***************************** */
	
		  /* echo "<br>smtp_protocol ".$smtp_protocol;
		  echo "<br>smtp_host ".$smtp_host;
      ...
      if ($Template_type == "Tier_notification") {
        ...
        if ($Email_content['Process'] == "Upgrade") {
          ...
          $Contents = 'Amazing News!!! You have successfully been upgraded to a new tier.
							You have moved from the ' . $Email_content['Old_Tier_name'] . ' Membership to the ' . $Email_content['New_Tier_name'] . ' Membership <br><br>Now you have access to earn more benefits in our Loyalty Program.
        ...
        if ($Email_content['Process'] == "Downgrade") {
          $Contents = 'You have not maintained the criteria for being a ' . $Email_content['Old_Tier_name'] . ' member hence you have therefore been moved to the ' . $Email_content['New_Tier_name'] . ' Tier.<br><br>To enable you move back to your previous tier, Kindly Visit us and spend to accumulate more points.';
		...
		include "./$App_folder_name/template/Auto_Tier_notification.php";

        $body = ob_get_contents();
        ob_end_clean();

        /*         * **********************************Variable Replace Code***************************** */
        $search_variables = array('$Customer_name', '$Todays_date', '$Member_website', '$Company_name', '$Gooogle_Play', '$iOs_app_store', '$Contents', '$Logo'); //
        $inserts_contents = array($Customer_name, $Todays_date, $Member_website, $Company_name, $Gooogle_Play, $iOs_app_store, $Contents,$Logo);
        $html = str_replace($search_variables, $inserts_contents, $body);
      ...
      if ($Template_type == "Points_Expiry") {
        ...
        $Total_Redeem_Points = ($Email_content['Total_Redeem_Points']);
        if ($Email_content['Days'] > 0) {
          $Contents = "Thank you for your continued patronage. We wanted to take a moment to remind you that " . $Email_content['Deduct_balance'] . " " . $Comapany_Currency . " loyalty points are set to expire in the next ". $Email_content['Days'] ." days.<br><br>
			We don't want you to miss out on any rewards and there's no better time than now to make the most out of your loyalty membership. So, why not treat yourself to something special with your points before they expire?
			<br><br>
			Thank you for your continued loyalty and we look forward to serving you in the future";
          ...
          $subject = "Notification: Expiry of Loyalty Points";
        } else {
        ...
        include "./$App_folder_name/template/Auto_points_expiry_notification.php";

        $body = ob_get_contents();
        ob_end_clean();

        /*         * **********************************Variable Replace Code***************************** */
        $search_variables = array('$Customer_name', '$Member_website', '$Company_name', '$Gooogle_Play', '$iOs_app_store', '$Contents', '$Img', '$Logo'); //
        $inserts_contents = array($Customer_name, $Member_website, $Company_name, $Gooogle_Play, $iOs_app_store, $Contents, $Img);
        $html = str_replace($search_variables, $inserts_contents, $body, $Logo);
	...
	if ($Template_type == "Meal_Top_Up") 
	{
      ...
      if ($company_details->Company_id == 14 && ( $Template_type == "Enroll_Freebies" || $Template_type == "Enroll" )) {
		...
		// $mail->addBCC('amitk@miraclecartes.com');
		// $mail->addBCC('nileshc@miraclecartes.com');
      }
      if ($Template_type == "Partner_payment") {
        // $this->CI->email->cc(array($Company_admin_email_id));
		$mail->addCC($Company_admin_email_id);
      }
      if ($Template_type == "Purchase_Return_Initiated") {
      ...
      if ($Template_type == "Publisher_purchased_miles_File") {
		...
		$mail->addCC($Email_content['Contact_email_id1']);
		$mail->addCC($Email_content['Contact_email_id2']);
      ...
      if ($Template_type == "Reconsolation_Error_File") {
		...
		$mail->addCC($company_details->Company_finance_email_id);
		$mail->addCC($Email_content['Contact_email_id1']);
		$mail->addCC($Email_content['Contact_email_id2']);
		$mail->addAttachment($Email_content['error_file_path'], '');
      }
      // if ($Template_type == "Communication" && $Email_content['VoucherQuantity'] > 1) {
      if ($Template_type == "Communication" && $Link_to_voucher==1) {
        
		$mail->addAttachment($Email_content['VouchersList_PDF'], '');
      }

      if ($Template_type == "Pos_json_error" ) {
        // $mail->addBCC('loyaltyapp@novacom.co.ke');
        $mail->addBCC('amitk@miraclecartes.com');
        $mail->addBCC('rakesh@miraclecartes.com');
		$mail->addAttachment($Email_content['Pos_error_file'], '');
      }
      ...
      // print_r($this->CI->email->print_debugger(), true);
		...
		if($Fcm_access_token != Null && $Fcm_token != Null)
		{
			...
			$To = $Fcm_token;	
			$action_url = $Base_url.$App_folder_name."/index.php/Cust_home/compose?Id=$customer_notification";
			
			$notifData = ['title'=>$Company_name,'body'=>$subject,'click_action'=>$action_url];
