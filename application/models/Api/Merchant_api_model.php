...
/******************Nilesh Start*****************/
   ...
   {		
		...
		{ 
			...
			{
				$where = '(a.Card_id2 = "'.$cid.'")';
			}
			else if($Mobile_number_as_card_id == 1)
			{
		...
		public function get_pos_redeem($cid,$compId,$phnumber,$Pin_type)
		{ 
			$company_details = $this->get_company_details($compId);
			$Mobile_number_as_card_id = $company_details->Use_mobile_number_as_card_id;
			$Physical_card_link = $company_details->Physical_card_link;
			...
			else if($Pin_number_as_card_id == 1)
			{
				...
				$this->db->join('igain_tier_master as b','a.Tier_id = b.Tier_id');
				$where = '(a.pinno = "'.$cid.'")';
				$this->db->where(array('a.Company_id' => $compId,'a.User_activated' => '1','a.User_id' => '1','a.Pin_type' => $Pin_type));
		...
		function cust_details_from_card_earn($Company_id,$Membership_id,$phnumber,$Pin_type)
		{ 
			...
			if($Physical_card_link == 1)
			{
				$where = '(a.Card_id2 = "'.$Membership_id.'")';
			}
			else if($Mobile_number_as_card_id == 1)
			{
			...
			else if($Pin_number_as_card_id == 1)
			{
				...
				$this->db->where(array('a.Company_id' => $Company_id,'a.User_activated' => 1,'a.User_id' => 1,'a.Pin_type' => $Pin_type));
		...
		/**********************Prestashop New Fun**********************/
		function fastenroll($Customer_topup,$Company_id,$card_decsion,$Logged_user_enrollid,$Country_id,$Card_id,$timezone_entry,$Customer_name,$Email_id,$Phone_no,$Customer_address,$Alternative_address,$Physical_card_id)
		{
			...
			$User_pwd = App_string_encrypt($pinno);
			$Customer_address = App_string_encrypt($Customer_address);
			$Alternative_address = App_string_encrypt($Alternative_address);
			...
			$data['Card_id2'] = $Physical_card_id;
			$data['Tier_id'] = $TierID;
			...
			$data['Current_address'] = $Customer_address;
			$data['Alternative_address'] = $Alternative_address;
		...
		function get_discount_value($Itemcategory_id,$ItemCode,$Item_price,$Company_id,$delivery_outlet,$cust_enroll,$Tier_id,$grandTotal,$API_flag_call)
		{
			...
			$this->db->join('igain_new_discount_rule_child as B' ,'A.Discount_code = B.Discount_code AND A.Company_id = B.Company_id','LEFT');
		...
		function get_member_discount_vouchers($Card_id,$Company_id)
		{
			...
			$this->db->where(array('Used_flag' => '0','Valid_till >=' => $today));
			$query = $this->db->get('igain_giftcard_tbl');	
	...
	function Get_Sent_Product_Voucher_Details($Gift_card_id,$Cust_enrollement_id,$Company_id)
	{
		$this->db->select("*");
		$this->db->from('igain_company_send_voucher');
		$this->db->where(array('Enrollement_id'=> $Cust_enrollement_id,'Company_id'=> $Company_id,'Voucher_code'=> $Gift_card_id));		
	...
	function Get_vouchers($Card_id,$Company_id)
	{
		...
		$this->db->select('*');
		$this->db->from('igain_giftcard_tbl');
		$this->db->where(array('Company_id'=> $Company_id,'Card_id'=> $Card_id,'Valid_till >='=>$Todays_date,'Used_flag'=>0,'Payment_Type_id !=' => 997));
		$sql=$this->db->get();
		if($sql->num_rows()>0)
		{
			foreach ($sql->result() as $row)
	...
	function get_member_product_vouchers($Card_id,$Company_id,$item_code)
	{
		...
		$this->db->where(array('igain_giftcard_tbl.Used_flag' => '0','igain_giftcard_tbl.Valid_till >=' => $today));
...
//*****stamp flag item offers ***********
	function Validate_gift_card($Company_id,$Giftcard_No)
	{
		...
		$this->db->where(array('Card_balance >' => '0','Valid_till >=' => $today,'Used_flag =' => 0));
	...
	public function get_redeem_code_details($Company_id,$Enrollement_id,$Membership_id)
	{
		$this->db->select("*");
		$this->db->from("igain_cust_earn_redeem_code_log");
		$this->db->where(array('Company_id' => $Company_id, 'Enrollement_id' => $Enrollement_id, 'Card_id' =>$Membership_id,'Pin_type_id' => 0, 'pinno_used' => 0));
		$this->db->limit('1');
	...
	{		
		$query =  $this->db->select('Bill_no')
			  ->from('igain_transaction')
			  ->where(array('Bill_no' => $Bill_no, 'Company_id'=> $Company_id )) ->get();
			return $query->num_rows();
	}
}
?>