...
//***********************************************Akshay Starts************************************
   ...
   public function wallet_debit_trans_list($limit,$start,$Logged_user_id,$Seller_id,$Company_id)
	...
	public function check_outlet_bill_no($Bill_no,$Company_id,$Outlet_id)
	{
		...
		$this->db->from('igain_transaction');
		$this->db->where(array('Manual_billno' => $Bill_no, 'Company_id' => $Company_id, 'Seller' =>$Outlet_id));
...
//*****************************************Sandeep Work Starts *************************************************
   ...
   { 
	   ...
	   $data["Card_value"] = $this->input->post("gif_balance");
	   ...
	   if($data["Card_id"] > 0)
	   {
		   ...
		   $data77['Trans_type'] = 4;
		   $data77['Purchase_amount'] = $this->input->post("gif_balance");
		   $data77['Mpesa_Paid_Amount'] = $this->input->post("gif_balance");
		   $data77['Mpesa_TransID'] = $this->input->post("Mpesa_code");
		   $data77['Payment_type_id'] = $this->input->post("Payment_type");
		   $data77['Trans_date'] = date("Y-m-d");
		   $data77['Enrollement_id'] = $Enrollement_id;
		   $data77['Card_id'] = $data["Card_id"];
		   $data77['GiftCardNo'] = $this->input->post("gif_number");
   ...
   public function loyalty_transaction_count($Logged_user_id,$Seller_id,$Company_id)
	...
	public function loyalty_transactions_list($limit,$start,$Logged_user_id,$Seller_id,$Company_id)
	{
		...
		if($Logged_user_id == 2)
		{
			$this->db->where(array('B.Trans_type' => '2', 'A.Company_id' => $Company_id, 'B.Create_user_id' => $Seller_id, 'B.Source' => 1));
		}
		else
		{
			$this->db->where(array('B.Trans_type' => '2', 'A.Company_id' => $Company_id, 'B.Create_user_id' => $Seller_id, 'B.Source' => 1));
...
/********************Nilesh start for push failed orders 04-12-2019**************************/
	...
	function get_earn_code_info($Earn_code,$Company_id,$Membership_id)
	{
		$this->db->select('*');
		$this->db->from('igain_cust_earn_redeem_code_log');
		$this->db->where(array('Pin' => $Earn_code, 'Company_id' => $Company_id, 'Card_id' => $Membership_id));
		$query = $this->db->get();
		...
		{			
			foreach ($query->result() as $row)
			{
               $result[] = array("Code" => $row->Pin,"Card_id" => $row->Card_id, "pinno_used" => $row->pinno_used, "creation_date_time" => $row->pinno_creation_date_time, "expiry_date_time" => $row->pinno_expiry_date_time);
            }			
			return json_encode($result);
		}
		else
	...
	function Validate_member_code($Earn_code,$Company_id,$Membership_id)
	{
		$this->db->select('*');
		$this->db->from('igain_cust_earn_redeem_code_log');
		$this->db->where(array('Pin' => $Earn_code, 'Company_id' => $Company_id, 'Card_id' => $Membership_id, 'pinno_used' => 0));
		$query = $this->db->get();
		if($query -> num_rows() > 0)
		{
			return $query->row();
	...
	public function update_customer_earn_code($Customer_earn_code,$Company_id,$cardId,$CustomerData)
	{
		$CustomerData1 = array(
					'pinno_used' => 1
				);
		$this->db->where(array('Company_id' => $Company_id, 'Card_id' => $cardId, 'Pin' => $Customer_earn_code));
		$this->db->update("igain_cust_earn_redeem_code_log", $CustomerData);
		...
		$this->db->where(array('Company_id' => $Company_id, 'Card_id' => $cardId, 'pinno' => $Customer_earn_code));
		$this->db->update("igain_enrollment_master", $CustomerData1);
	...
	function Get_voucher_info($VoucherNumber,$MemberCardID,$Company_id)
	{
		...
		$this->db->where(array('Gift_card_id' => $VoucherNumber, 'Card_id' => $MemberCardID, 'Company_id' => $Company_id, 'Valid_till >= ' => $Todays_date,'Used_flag' => 0));
	...
	function update_voucher_balance($Gift_card_id,$Card_balance,$Company_id,$Update_user_id)
	{
		$data = array(
					'Card_balance' => $Card_balance,
					'Used_flag' => 1,
