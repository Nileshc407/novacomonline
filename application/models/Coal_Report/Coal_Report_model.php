...
class Coal_Report_model extends CI_model
{
	...
	{	
		...
		$this->db->select('First_name,Middle_name,Last_name,IT.Enrollement_id,IT.Trans_id,IT.Trans_date,Purchase_amount,Redeem_points,Paid_amount,Topup_amount,Bill_no,Seller,IT.Card_id as Membership_ID,IT.Company_id,IT.Card_id2 as Transfer_to,Transfer_points,TM.Tier_name,IT.Remarks as Remarks,TT.Trans_type,Trans_type_id,Item_code,Quantity ');//Merchandize_item_name
	...
	function get_alk_cust_trans_details_exl($Company_id,$From_date,$To_date,$Enrollement_id,$transaction_type_id,$Tier_id)
	{
		$From_date=date("Y-m-d",strtotime($From_date));
		$To_date=date("Y-m-d",strtotime($To_date));
		$this->db->select('IT.Trans_date,CONCAT(First_name,Middle_name,Last_name) AS Member_name,TM.Tier_name,IT.Card_id as Membership_ID,TT.Trans_type,Bill_no,Quantity,Purchase_amount,Redeem_points,IT.Card_id2 as Gift_to,Transfer_points as Gift_amount,IT.Remarks as Remarks ');
		$this->db->from('igain_transaction as IT');
	...
	function get_merchant_report($start_date,$end_date,$Company_id,$seller_id,$transaction_type_id,$report_type,$login_enroll)
    {
		...
		if($report_type == 1)
		{
			...
			$fields = array(
						...
						'Source' => array('type' => 'INT','constraint' => '11','DEFAULT' => 0),
		...
		if($report_type == 0)
		{
			...
			if($transaction_type_id == 0)
			{
				$this->db->select('A.Remarks,A.Trans_id, A.Manual_billno, A.Enrollement_id, A.Seller_name, A.Topup_amount, A.GiftCardNo, A.Purchase_amount, A.Redeem_points, A.balance_to_pay, A.Card_id, A.Bill_no, A.Loyalty_pts,A.Voucher_no,A.Voucher_discount,A.Coalition_Loyalty_pts, A.Seller, A.Trans_date, B.Trans_type, B.Trans_type_id, C.Company_name, D.First_name, D.Middle_name, D.Last_name,Quantity,A.Channel_id,A.Source');
				$this->db->from('igain_transaction as A');
				$this->db->join('igain_transaction_type as B', 'B.Trans_type_id = A.Trans_type');
				$this->db->join('igain_enrollment_master as D', 'D.Enrollement_id = A.Enrollement_id');
				$this->db->join('igain_company_master as C', 'C.Company_id = D.Company_id');
			...
			if($transaction_type_id != 0)
			{
				...
				{
					$this->db->select('AA.Remarks,AA.Trans_id, AA.Manual_billno, AA.Enrollement_id, AA.Seller_name, AA.Topup_amount, AA.GiftCardNo, AA.Purchase_amount, AA.Redeem_points, AA.balance_to_pay, AA.Card_id, AA.Bill_no, AA.Loyalty_pts,AA.Voucher_no,AA.Voucher_discount, AA.Coalition_Loyalty_pts,AA.Seller, AA.Trans_date, BB.Trans_type, BB.Trans_type_id, CC.Company_name,Quantity,A.Channel_id,AA.Source');
					$this->db->from('igain_transaction as AA');
					$this->db->join('igain_transaction_type as BB', 'BB.Trans_type_id = AA.Trans_type');
					$this->db->join('igain_company_master as CC', 'CC.Company_id = AA.Company_id');
					$this->db->where(array('AA.Company_id' => $Company_id, 'AA.Trans_type' => '4'));
	...
	public function get_seller_excel_report_details($temp_table,$Report_type)	//,$limit,$start
	{
		if($Report_type == 1)
		{
			...
			$this->db->select('sellerName as Outlet,Trans_type,Source,purchase_amt as Purchase_amt,top_up as Bonus_points,reedem_pt as Reedem_pts,Voucher_redeemed,Voucher_amount');
			...
			if ($query->num_rows() > 0)
			{
				foreach ($query->result() as $row)
				{
					...
					if($row->Source==0)
					{
						$row->Source="POS Transactions";
					}
					else
					{
						$row->Source="Offline POS Transactions";
					}
	...
	/*
		...
		*/
		...
		$this->db->select('First_name,Middle_name,Last_name,IT.Enrollement_id,IT.Trans_id,IT.Trans_date,Purchase_amount,Redeem_points,Paid_amount,Topup_amount,Manual_billno as Bill_no,TT.Trans_type_id,Seller,IT.Card_id as Membership_ID,IT.Company_id,IT.Card_id2 as Transfer_to,Transfer_points,Expired_points,TM.Tier_name,IT.Remarks as Remarks,TT.Trans_type ');
		$this->db->from('igain_transaction as IT');
		$this->db->join('igain_enrollment_master as IE','IT.Enrollement_id=IE.Enrollement_id');
		$this->db->join('igain_transaction_type as TT','IT.Trans_type=TT.Trans_type_id');
		$this->db->join('igain_tier_master as TM','IE.Tier_id=TM.Tier_id');
	...
	function get_cust_trans_summary_all($Company_id,$Enrollement_id,$start_date,$end_date,$transaction_type_id,$Tier_id,$start,$limit,$Enable_company_meal_flag)
	{
		...
		$this->db->select('IE.First_name,IE.Middle_name,IE.Last_name,IT.Card_id AS Membership_ID,TT.Trans_type,SUM(Purchase_amount) as Total_Purchase_Amount,SUM(Redeem_points) AS Total_Redeem,SUM(Paid_amount) AS Total_Paid_amount,SUM(Topup_amount) as Total_Credit_topup_Points,SUM(Transfer_points) AS Total_Transfer_Points,SUM(Expired_points) AS Total_Expired_points,Tier_name,IT.Enrollement_id');
	...
	function get_cust_trans_details_reports($Company_id,$From_date,$To_date,$Enrollement_id,$transaction_type_id,$Tier_id,$Enable_company_meal_flag)
	{
		...
		$this->db->select('IT.Trans_date,First_name,Last_name,IT.Card_id AS Membership_ID,Tier_name,TT.Trans_type,IT.Source,Manual_billno as  Bill_no,Purchase_amount,Redeem_points,Paid_amount,Topup_amount,Mpesa_TransID,IT.Card_id2 as Transfer_to,Transfer_points,Expired_points,Remarks,Seller_name as Transaction_by');
		$this->db->from('igain_transaction as IT');
		$this->db->join('igain_enrollment_master as IE','IT.Enrollement_id=IE.Enrollement_id');
		$this->db->join('igain_transaction_type as TT','IT.Trans_type=TT.Trans_type_id');
		$this->db->join('igain_tier_master as TM','IE.Tier_id=TM.Tier_id');
		...
		if($sql51 -> num_rows() > 0)
		{
			...
			foreach ($sql51->result() as $row)
			{
				...
				$this->db->select('IT.Trans_date,First_name,Middle_name,Last_name,IT.Card_id AS Membership_ID,Tier_name,TT.Trans_type,Bill_no,Purchase_amount,Redeem_points,Paid_amount,Topup_amount,IT.Card_id2 as Transfer_to,Transfer_points,Remarks');
				...
				if($row->Trans_type == 'PoS Transaction')
				{
					if($row->Source==0 )
					{
						$row->Source="POS Transactions";
				...
				{
					$row->Source="-";
					if($row->Trans_type == 'Credit - Topup Points' && $row->Mpesa_TransID !='')
					{
						$row->Source="Mobile APP";
				...
				if($row->Trans_type == 'Credit - Topup Points' && $row->Mpesa_TransID !='')
				{
					$row->Remarks="Top_Up-".$row->Mpesa_TransID;					
				}
				if($Enable_company_meal_flag==1 && $row->Trans_type == 'Bonus Points')
				{
					$row->Trans_type = 'Meal Topup';
