...
//************* akshay work start *************************************	
   ...
   function fastenroll($Customer_topup,$ref_Customer_enroll_id,$State,$City,$Card_id)
	...
	function check_card_id2($cardid,$Company_id)
	{
		$query =  $this->db->select('Card_id2')
				   ->from('igain_enrollment_master')
				   ->where(array('Card_id2' => $cardid, 'Company_id' => $Company_id))->get();
		return $query->num_rows();
	}
	...
	function check_phone_no1($Phone_no1,$Company_id,$Country_id)
	{
		/* $this->db->select('phonecode');
		$this->db->from('igain_country_master');
		$this->db->where('id',$Country_id);
		...
		$Phone_no = $phonecode."".$Phone_no;
		 */
		$query =  $this->db->select('Phone_no1')
				   ->from('igain_enrollment_master')
				   ->where(array('Phone_no1' => App_string_encrypt($Phone_no1), 'Company_id' => $Company_id))->get();
	...
	function check_homeEmailId($homeEmailId,$Company_id,$UserID)
	{	 
		if($UserID ==  1)
		{  
			$query =  $this->db->select('User_email_id1')
			   ->from('igain_enrollment_master')
			->where(array('User_id' => '1','User_email_id1' => App_string_encrypt($homeEmailId),'Company_id' => $Company_id))->get();
		}
		else
		{
			$query =  $this->db->select('User_email_id1')
			   ->from('igain_enrollment_master')
			->where(array('User_email_id1' => App_string_encrypt($homeEmailId),'Company_id' => $Company_id, 'User_id !=' => '1'))->get();	
		}
		//echo $this->db->last_query();	   
		return $query->num_rows();
	}
    ...
    { 				
		...
		$email_id = $this->input->post('userEmailId');
		$Card_id2 = $this->input->post('card_id2');
		...
		$data['Alternative_address'] =  App_string_encrypt($this->input->post('alternativeAddress')); 
		...
		if($this->input->post('dob')==date('m/d/Y')){$data['Date_of_birth'] = '1970-01-01';}
		...
		if($User_type_id == 1)
		{
			...
			// $data['Current_balance'] = $Customer_topup;
			$data['Card_id'] = $Card_id;
			$data['Card_id2'] = $Card_id2;
			
			$TierID = $this->input->post('member_tier_id');
			$data['Tier_id'] = $TierID;
			$data['Expire_monthend_balance']=$this->input->post('Expire_monthend_balance');
			$RefrenceD = 0;
			...
			/* 08-03-2021 */
			if($CMPresultis->Club_flag == 1)
			{
				$data['User_email_id1'] = App_string_encrypt($this->input->post('homeEmailId'));
				$data['Phone_no1'] = App_string_encrypt($this->input->post('phno1'));
				$data['Nationality'] = $this->input->post('Nationality');
				$data['Nationality1'] = $this->input->post('Nationality1');
   ...
   {	   
	...
	public function Selected_company_customer_staff_list($limit,$start,$Company_id)
	{
		...
		$this->db->order_by('Enrollement_id','desc'); 
		$this->db->from('igain_enrollment_master'); 
		$this->db->where(array('Sub_seller_admin'=>'0','User_activated' => '1','Company_id' => $Company_id));
		$this->db->where("User_id IN('1','7')");
		$company_sql= $this->db->get();
		if($company_sql->num_rows() > 0)
		{
			   foreach ($company_sql->result() as $row)
