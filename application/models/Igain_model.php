...
//**************** sandeep work start ***********************************	
	...
	function get_company_sellers1($Company_id)
	{
	  $this->db->select("Enrollement_id,First_name,Middle_name,Last_name,Current_balance,Super_seller");
	  $this->db->from('igain_enrollment_master');
	  $this->db->where(array('User_id' => '7','User_activated' => '1','Company_id' => $Company_id));
	  $sql = $this->db->get();
	  // echo $this->db->last_query();
...
/*************amit get code decode master 13-09-2017*********end*******/
	...
	function validate_member($cardid,$Company_id)
	{
		...
		//OR First_name like"'.$cardid.'" OR Last_name like"'.$cardid.'"
		$this->db->where(array('Company_id' => $Company_id,'User_id' => 1,'User_activated' => 1));
		$query = $this->db->get();
