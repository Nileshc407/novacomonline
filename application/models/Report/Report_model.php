...
/*********** Sandeep Work Start ****************************/
	...
	/*
		...
		*/
		...
		$this->db->select('First_name,Middle_name,Last_name,IT.Enrollement_id,IT.Trans_id,IT.Trans_date,Purchase_amount,Redeem_points,Paid_amount,Topup_amount,Loyalty_pts,Bill_no,TT.Trans_type_id,Seller,TT.Trans_type,IT.Card_id as Membership_ID,IT.Company_id,IT.Card_id2 as Transfer_to,Transfer_points,TM.Tier_name,IT.Remarks as Remarks,balance_to_pay,Item_code,Quantity,Voucher_no,Voucher_status,Expired_points');
	...
	function get_cust_trans_details_reports($Company_id,$From_date,$To_date,$Enrollement_id,$transaction_type_id,$Tier_id)
	{
		...
		$this->db->select('IT.Trans_date,First_name,Middle_name,Last_name,IT.Card_id AS Membership_ID,Tier_name,TT.Trans_type,Bill_no,Purchase_amount,Paid_amount,Loyalty_pts,Redeem_points,Item_code,Quantity,Voucher_no,Voucher_status,Topup_amount,IT.Card_id2 as Transfer_to,Transfer_points,Remarks,Expired_points');
		$this->db->from('igain_transaction as IT');
		$this->db->join('igain_enrollment_master as IE','IT.Enrollement_id=IE.Enrollement_id');
		$this->db->join('igain_transaction_type as TT','IT.Trans_type=TT.Trans_type_id');
		$this->db->join('igain_tier_master as TM','IE.Tier_id=TM.Tier_id');
	...
	public function get_cust_expiry_report($Company_id,$start_date,$end_date)
	{
...
/******************************Nilesh end partner merchandize ctelogue report******************************/	
	...
	{  
		...
		$this->db->select('First_name,Middle_name,Last_name,IT.Enrollement_id,IT.Trans_id,IT.Trans_date,From_Beneficiary_company_name,To_Beneficiary_company_name,Paid_amount,To_Beneficiary_cust_name,Bill_no,TT.Trans_type_id,Seller,IT.Card_id as Membership_ID,TT.Trans_type,IT.Company_id,IT.Card_id2 as Transfer_to,Topup_amount as Recived_point,Transfer_points,TM.Tier_name,IT.Remarks as Remarks');
		$this->db->from('igain_transaction as IT');
		$this->db->join('igain_enrollment_master as IE','IT.Enrollement_id=IE.Enrollement_id');
		$this->db->join('igain_transaction_type as TT','IT.Trans_type=TT.Trans_type_id');
		$this->db->join('igain_tier_master as TM','IE.Tier_id=TM.Tier_id');
	...
	{  
		...
		$this->db->select('IT.Trans_date,First_name,Last_name,IT.Card_id AS Membership_ID,From_Beneficiary_company_name,To_Beneficiary_company_name,To_Beneficiary_cust_name,IT.Card_id2 as Transfer_to,Transfer_points,Topup_amount as Recived_point,Remarks');
		$this->db->from('igain_transaction as IT');
		$this->db->join('igain_enrollment_master as IE','IT.Enrollement_id=IE.Enrollement_id');
		$this->db->join('igain_transaction_type as TT','IT.Trans_type=TT.Trans_type_id');
		$this->db->join('igain_tier_master as TM','IE.Tier_id=TM.Tier_id');
		...
		if($sql51 -> num_rows() > 0)
		{
			...
			foreach ($sql51->result() as $row)
			{
				...
				$this->db->select('IT.Trans_date,First_name,Middle_name,Last_name,IT.Card_id AS Membership_ID,Tier_name,From_Beneficiary_company_name,To_Beneficiary_company_name,To_Beneficiary_cust_name,IT.Card_id2 as Transfer_to,Transfer_points,Topup_amount as Recived_point,Remarks');
			...
			// echo $this->db->last_query(); die;
				...
				foreach($query60 as $row60)
				{
					...
					$total_store_visits = $this->get_member_store_visits($row60->Membership_ID,$Company_id, $start_date, $end_date,$Outlet_id);
							...
							$row60->Voucher_amt = round($discount['totalVoucherDiscount']);
							$row60->Discount_amt = round($discount['totalDiscount']);
	...
	function Get_outlet_pos_discount_summary($Company_id, $start_date, $end_date, $transaction_from,$Outlet_id)
	{
		$start_date=date("Y-m-d 00:00:00", strtotime($start_date));
		$end_date=date("Y-m-d 23:59:59", strtotime($end_date));
	...
	function Get_items_summary_trans_records($Company_id, $start_date, $end_date, $Category,$Item,$MembershipID)
	{
		...
		$this->db->select("t.Card_id as MembershipID,CONCAT(e.First_name,' ',e.Last_name) as Member_name,Phone_no,Item_code,Merchandize_item_name,SUM(Quantity) AS Sold_Quantity,SUM(t.Purchase_amount) as Item_value,c.Merchandize_category_id,Merchandize_category_name as Menu_group,COUNT(Item_code) AS Times_purchased,Purchase_amount as Cat_value"); 
		...
		// if($Category != 0){	$this->db->where_in('t.Item_code',$Item); }
			$this->db->where_in('t.Item_code',$Item); 
	...
	function Get_cat_summary_trans_records($Company_id, $start_date, $end_date, $Category,$Item,$MembershipID)
	{
		...
		// $this->db->select("Card_id as MembershipID,CONCAT(e.First_name,' ',e.Last_name) as Member_name,Phone_no,Merchandize_category_name as Menu_group,COUNT(Item_code) AS Times_purchased,SUM(t.Purchase_amount) as Menu_group_value"); Cat_Times_purchased
		...
		$query58 = $this->db->get();
		// echo $this->db->last_query(); die;Cat_Times_purchased
		if($query58->num_rows() > 0)
		{
			$lv_MembershipID='0';
			$lv_category_id='0';
	...
	function get_category_by_subadmin($Company_id,$Enrollement_id)
	{
		...
		$this->db->where(array('m.Company_id' => $Company_id,'m.Active_flag' => '1','c.Active_flag' => '1'));
		if($Enrollement_id != 0){
			$this->db->where(array('m.Seller_id' => $Enrollement_id));
		}
	...
	function Get_Linked_Items_for_category($Seller_id,$Merchandize_category_id,$Company_id)
	{
		...
		$this->db->where(array('A.Active_flag'=>1,'A.Company_id'=>$Company_id,'A.Merchandize_item_type <>' => 119));
		if($Seller_id != 0){
			$this->db->where(array('A.Seller_id'=>$Seller_id));
		}
		if($Merchandize_category_id != 0){
			$this->db->where(array('A.Merchandize_category_id'=>$Merchandize_category_id));
		}
	...
	public function get_cust_info($Cust_name,$Company_id) 
	{        
		$cust_name = explode(" ",$Cust_name);
		$this->db->select("First_name,Last_name,User_email_id,Enrollement_id,Card_id,Phone_no");
		$this->db->like("First_name", $cust_name[0]);
		$this->db->like("Last_name", $cust_name[1]);
		$this->db->where(array('User_id' => '1','User_activated' => '1','Company_id' => $Company_id));	//,'Card_id' => '0'
        $query = $this->db->get('igain_enrollment_master');
		return $query->result_array();
    }
	function Get_voucher_report_records($Company_id, $start_date, $end_date, $Status,$Outlet_id,$Stamp_flag,$membership_id)
	{
		$start_date=date("Y-m-d 00:00:00", strtotime($start_date));
		$end_date=date("Y-m-d 23:59:59", strtotime($end_date));
		...
		$this->db->select("DISTINCT(Gift_card_id) as Voucher_no,CONCAT(First_name,' ',Last_name) as Issued_to,g.Card_id as MembershipID,g.Create_date as Issued_on,Card_value as Value,Card_balance,g.Discount_percentage,g.Update_user_id,g.Update_date,g.Payment_Type_id,g.Seller_id,g.Valid_till"); //,Voucher_name
		/* if($Status == 2){
			$this->db->select("DISTINCT(Gift_card_id) as Voucher_no,CONCAT(First_name,' ',Last_name) as Issued_to,g.Card_id as MembershipID,g.Create_date as Issued_on,Bill_no as Order_no,Manual_billno as Pos_Bill_No,Card_value as Value,Card_balance,g.Discount_percentage,g.Update_user_id,g.Update_date,g.Payment_Type_id,g.Seller_id");
		...
		if($Stamp_flag==1)//For Stamp Report  
		{
			...
			$this->db->where('v.Quantity != 0 ');
		} 
		if($Status == 2){
			$this->db->join('igain_transaction as t', 'g.Gift_card_id = t.Voucher_no AND g.Company_id = t.Company_id','LEFT'); 
		}
		
		// $this->db->join('igain_company_send_voucher as v', 'v.Voucher_code = g.Gift_card_id AND v.Company_id = g.Company_id'); 
		// $this->db->join('igain_company_voucher_catalogue as vc', 'vc.Voucher_id = v.Voucher_id AND v.Company_id = vc.Company_id'); 
		
		$this->db->where('g.Card_id != "0" ');
		if($membership_id != '' ){
		...
		{
			if($this->input->post("VoucherType")!=0)
			{
				$this->db->where('g.Payment_Type_id', $this->input->post("VoucherType"));
			}
		}
		
		 if($Status == 2){
			$this->db->where('g.Card_balance = "0.00" ');
			$this->db->where('g.Update_date BETWEEN "'.$start_date.'" AND  "'.$end_date.'" ');
		}
		...
		if($Status == 0){
			$this->db->where('(g.Create_date BETWEEN "'.$start_date.'" AND  "'.$end_date.'" ');
			$this->db->or_where('g.Update_date BETWEEN "'.$start_date.'" AND  "'.$end_date.'" )');
		}
		...
		if($query58 != NULL )
		{
			foreach($query58 as $row58)
			{
				...
				$this->db->limit(1,0);
				$sql631 = $this->db->get();
				//echo $this->db->last_query();Issud_Outlet
				...
				{
					if($Status == 1 && $row58->Valid_till < $Todays_date)
					{
						continue;
					}
					if($row58->Card_balance != '0.00' && $row58->Valid_till > $Todays_date)
					{
				...
				// if(($Status == 0 || $Status == 3) && $row58->Card_balance != '0.00' && $row58->Valid_till < $Todays_date)
				if($row58->Card_balance != '0.00' && $row58->Valid_till < $Todays_date)
				{
					if($Status == 0 || $Status == 3)
					{
						$row58->Status = 'Expired';
						$row58->Used_Outlet = '-';
						$row58->Used_date = '-';
					}
					
				}
				if($Status == 3 && $row58->Valid_till > $Todays_date)
				{
					continue;
				}
	...
	function Get_Voucher_report_summary($Company_id, $start_date, $end_date,$membership_id)
	{
		...
		if($query58 != NULL )
		{
			...
			{
					$row->Total_Issued_vouchers = ($row->Total_Used_vouchers + $row->Total_Issued_vouchers);
	...
	function get_cust_trans_summary_tobe_expiry_monthly($Company_id,$Points_expiry_period,$Days)
	{
		$Days=($Points_expiry_period-$Days);
		$last_month = date("Y-m-d",strtotime("-$Days days"));
		$From_month = date("Y-m-d",strtotime("-$Points_expiry_period days"));
		$Todays_date = date("Y-m-d");
		
		$this->db->select('T.Company_id,T.Enrollement_id,User_email_id,T.Card_id as Membership_ID,First_name,Last_name,(Current_balance- (Blocked_points + Debit_points) ) as Total_Balance,Trans_date,SUM(Total_topup_amount+Total_loyalty_pts-Knockout_points) as TobeExpire_pts');
		...
		public function Get_Customer_tier_report($Company_id, $start_date, $end_date, $Up_down_flag, $Enrollement_id)
	{
		$start_date=date("Y-m-d 00:00:00", strtotime($start_date));
		$end_date=date("Y-m-d 23:59:59", strtotime($end_date));
		
		$query = $this->db->select("e.Enrollement_id,t.Update_date,e.Card_id,First_name,Last_name,User_email_id,Phone_no,Tier_name as Current_tier,Old_Tier_name as Previous_tier,Process as Status,Cumulative_Points_Accumlated,Cumulative_Trans,Cumulative_Spend,Current_balance");
		$query = $this->db->from("igain_customer_tier_auto_process_log as t");
		$query = $this->db->join("igain_enrollment_master as e","t.Enrollement_id=e.Enrollement_id");
		$query = $this->db->where("t.Update_date Between '$start_date'  AND '$end_date' ");
		$query = $this->db->where("t.Company_id",$Company_id);
		$query = $this->db->order_by('t.Update_date','desc');
		
		if($Up_down_flag!='Both')
		{
			$query = $this->db->where("Process",$Up_down_flag);
		}
		if($Enrollement_id!='')
		{
			$query = $this->db->where("t.Enrollement_id",$Enrollement_id);
		}
		$query = $this->db->get();
		...
		if ($query->num_rows() > 0)
		{
			foreach ($query->result() as $row)
			{
				...
				if($row->Cumulative_Trans==0)
				{
					$row->Cumulative_Trans='-';
				}
				if($row->Cumulative_Spend==0.00)
				{
					$row->Cumulative_Spend='-';
				}
				if($row->Cumulative_Points_Accumlated==0)
				{
					$row->Cumulative_Points_Accumlated='-';
				}
				$data[] = $row;
			}
			return $data;
	...
	function Get_loyalty_summary_trans_records($Company_id,$end_date, $transaction_from,$Outlet_id,$Redemptionratio,$Channel_id)
	{
		$end_date=date("Y-m-d 23:59:59", strtotime($end_date));

		
		$this->db->select("Seller,Trans_date as Till_date,CONCAT(e.First_name,' ',e.Last_name) as Outlet,t.Trans_type as Channel,SUM(t.Purchase_amount) as Bill_amount,sum(t.Paid_amount) as Paid_amount,sum(t.Loyalty_pts) as Earn_points,sum(t.Redeem_points) as Redeem_points,(sum(t.Loyalty_pts)-sum(t.Redeem_points)) as Loyalty_Points_Balance,t.Channel_id"); 
		...
		$this->db->where('t.Trans_date <= "'.$end_date.'" ');
		...
		if($query58 != NULL )
		{
			foreach($query58 as $row58)
			{
				$row58->Till_date=date("Y-m-d", strtotime($end_date));
				$data58[] = $row58;
			}
			return $data58;
		}
	...
	function Get_Company_loyalty_summary_trans_records($Company_id,$end_date, $transaction_from,$Channel_id)
	{
		$end_date=date("Y-m-d 23:59:59", strtotime($end_date));

		
		$this->db->select("Seller,Trans_date as Till_date,t.Trans_type as Channel,SUM(t.Purchase_amount) as Bill_amount,sum(t.Paid_amount) as Paid_amount,sum(t.Loyalty_pts) as Earn_points,sum(t.Topup_amount) as Bonus_points,sum(t.Redeem_points) as Redeem_points,sum(t.Expired_points) as Expired_points,(sum(t.Loyalty_pts)+sum(t.Topup_amount)-sum(t.Redeem_points)-sum(t.Expired_points)) as Loyalty_Points_Balance,t.Channel_id"); 
		...
		if($transaction_from > 0){
			// $this->db->where(array('t.Trans_type' => $transaction_from));
			$this->db->where_in('t.Trans_type',array(14,$transaction_from));
			if($transaction_from == 29){
			$this->db->where_in('t.Channel_id', $Channel_id);
			}
		}
		...
		{
			$this->db->where_in('t.Trans_type',array(1,2,12,29,14));
		}
		
		
		$this->db->where('t.Trans_date <= "'.$end_date.'" ');

		$this->db->group_by("t.Company_id");
	...
	public function get_stamp_data($Company_id,$from_date,$till_date,$card_id,$tier_id,$select_cust)
	{
		$start_date=date("Y-m-d 00:00:00", strtotime($from_date));
		$end_date=date("Y-m-d 23:59:59", strtotime($till_date));
		
		$item_code_array=array();
		
		$this->db->select("M.Company_merchandize_item_code,O.From_date,O.Till_date");
		$this->db->from("igain_offer_master as O");
		$this->db->join("igain_company_merchandise_catalogue as M","M.Company_merchandise_item_id = O.Company_merchandise_item_id AND M.Company_id = O.Company_id");
		$this->db->where(array('O.Company_id' => $Company_id,'O.Active_flag' =>1));
		$query59 = $this->db->get()->result();
		foreach($query59 as $row59){
			$item_code_array[]=$row59->Company_merchandize_item_code;	
		}
		// print_r($item_code_array);
		$this->db->select("t.Trans_date,e.Card_id,CONCAT(e.First_name,' ',e.Last_name) as Member_name,t.Manual_billno as Bill_no,SUM(t.Purchase_amount) as Purchase_amt ,SUM(t.Quantity-t.Free_item_quantity) as Stamp_collected,t.Seller_name as Outlet");
		$this->db->from("igain_transaction as t");
		$this->db->join("igain_enrollment_master as e","t.Enrollement_id = e.Enrollement_id");
		$this->db->where("t.Trans_date Between '$start_date'  AND '$end_date' ");
		$this->db->where(array('t.Company_id' => $Company_id,'t.Trans_type'=>2));
		$this->db->where_in('t.Item_code',$item_code_array);
		if($select_cust == 1){
			if($tier_id !=0){
				$this->db->where('e.Tier_id',$tier_id); 				
			}
		}
		else if($select_cust == 2){
			$this->db->where('t.Card_id',$card_id); 
		}
		$this->db->order_by('t.Trans_id' , 'desc');
		$this->db->group_by("t.Bill_no");
		$query = $this->db->get()->result();
