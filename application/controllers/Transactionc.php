...
error_reporting(0);
  ...
  class Transactionc extends CI_Controller {
        ...
        if ($_POST == NULL) {
          ...
          if ($this->input->post("topup_amt") == "" || $this->input->post("topup_amt") <= 0 || $this->input->post("topup_amt") == " ") {
           if($Daily_points_consumption_flag==1 || $Enable_company_meal_flag==1){ $this->session->set_flashdata("error_code", "Sorry, Meal Topup Failed. Invalid Data Provided!!");}else{ $this->session->set_flashdata("error_code", "Sorry, Issue Bonus Transaction Failed. Invalid Data Provided!!");}
			...
			if($this->input->post('Payment_type') == 9)
			{
				$Bank_name = $this->input->post("Bank_name1");
				$Branch_name = $this->input->post("Bank_code");
			}
			else
			{
            ...
            if (($result == true) && ($result2 == true) && ($result4 == true)) {
				if($Daily_points_consumption_flag==1 || $Enable_company_meal_flag==1){ $this->session->set_flashdata("success_code", "Meal Topup Transaction done Successfully!!");}else{ $this->session->set_flashdata("success_code", "Topup Transaction done Successfully!!");}
              
            } else {
					if($Daily_points_consumption_flag==1 || $Enable_company_meal_flag==1){ $this->session->set_flashdata("error_code", "Sorry, Meal Topup Transaction Failed!!");}else{ $this->session->set_flashdata("error_code", "Sorry, Topup Transaction Failed!!");}
	...
	public function check_outlet_bill_no() {
      $result = $this->Transactions_model->check_outlet_bill_no($this->input->post("Bill_no"), $this->input->post("Company_id"), $this->input->post("Outlet_id"));
    ...
    public function loyalty_transaction() {
      if ($this->session->userdata('logged_in')) {
		...
		$data['Use_member_code_as_card_id'] = $company_details->Use_pin_number_as_card_id;
        /* -----------------------Pagination--------------------- */
        $config = array();
              ...
              if (strlen($cardis) != 0) {
                if ($cardis != '0') {
                  if ($cardId == $cardis || $Phone_no == $phnumber) {
					...
					$data['Sub_sellers'] = $this->Igain_model->Get_sub_seller($Logged_user_enrollid);
    ...
    public function loyalty_purchase_transaction_ci() {  
      if ($this->session->userdata('logged_in')) {
		...
		$redemptionratio = $company_details2->Redemptionratio;
		$Use_pin_number_as_card_id = $company_details2->Use_pin_number_as_card_id;
        /**********************************************************/
        ...
        if ($_POST == NULL) {
          ...
          redirect('Transactionc/loyalty_transaction');
			...
			$Redeem_voucher_flag = $this->input->post("Redeem_voucher_flag");
			$VoucherRedeem = 0;
			if($Redeem_voucher_flag==1){ 
				$VoucherRedeem = $_REQUEST['VoucherRedeemAmount']; 
			}	 
          if ($this->input->post("purchase_amt") == "" || $this->input->post("purchase_amt") <= 0 || $this->input->post("purchase_amt") == " ") {
            ...
            $bal_pay = ($this->input->post("purchase_amt") - $redeem_amount - $VoucherRedeem);
			...
			if($Use_pin_number_as_card_id == 1) {
				$Customer_earn_code = $this->input->post("Earn_code");
				$result70 = $this->Transactions_model->Validate_member_code($Customer_earn_code,$Company_id,$cardId);
				if($result70 == Null)
				{
					$this->session->set_flashdata("error_code", "Customer Earn Code In Invalid..!");
					redirect('Transactionc/loyalty_transaction');
				}
			}
			...
			/*------Manual_bill_no Checking---12-08-2019--*/
				$result = $this->Transactions_model->check_outlet_bill_no($this->input->post("manual_bill_no"),$Company_id,$this->input->post("Outlet_id"));				
				if ($result > 0) {
            ...
            if (!(is_null($bal_pay)) && $go_ahead == 1) {
              ...
              if ($data['userId'] == 3) {
				...
				$Outlet_details = $this->Igain_model->get_enrollment_details($_REQUEST['Outlet_id']);
              ...
              foreach ($loyalty_prog as $prog) {
                ...
                if ($lp_type == 'PA') {
				  ...
				  $transaction_amt = $this->cheque_format($transaction_amt);
				   $transaction_amt = str_replace( ',', '', $transaction_amt);
                }
			 ...
			 if($_REQUEST['Redeem_voucher_flag']==1)
			  {
				  $VoucherNumber=$_REQUEST['VoucherNumber'];
				  if($VoucherNumber!='')
				  {
					 $result32x = $this->Transactions_model->update_voucher_balance($_REQUEST['VoucherNumber'],0, $Company_id,$data['enroll']); 
				  }
			  }
			  else
			  {
			  ...
			  if($Use_pin_number_as_card_id == 1) 
			  {
				  $CustomerCodeData = array('Outlet_id' => $Outlet_id,
											'Pos_bill_no' => $manual_bill_no,
											'pinno_used' => 1
											);
				$result070 = $this->Transactions_model->update_customer_earn_code($Customer_earn_code,$Company_id,$cardId,$CustomerCodeData);
			  }
              $billno_withyear = $str . $bill_no;
              ...
              $Email_content = array(
				  ...
				  'Transaction_date' => $lv_date_time,
				  'Orderno' => $bill,
                  'Pos_billno' => $manual_bill_no,
                  'Purchase_amount' => $purchase_amt,
                  'subtotal' => $purchase_amt,
                  'Redeem_points' => $reedem,
                  'Cust_wish_redeem_point' => $reedem,
                  'EquiRedeem' => $reedem,
                  'Payment_by' => $payment_by,
                  'Balance_to_pay' => $balance_to_pay,
                  'grand_total' => $balance_to_pay,
                  'Total_loyalty_points' => $total_loyalty_points,
                  'total_loyalty_points' => $total_loyalty_points,
                  'Symbol_currency' => $Symbol_currency,
				  'Symbol_of_currency' => $Symbol_currency, 
				  'Delivery_type' => 2, 
				  'DeliveryOutlet' => $Outlet_id, 
				  'delivery_outlet' => $Outlet_name, 
                  'GiftCardNo' => $GiftCardNo,
			  ...
			  if ($_POST == NULL) {
				...
				if ($Redeem_type == 1) { //Normal
				  ...
				  $post_data = array(
					  ...
					  'Company_id' => $Company_id,
					  'Redeem_points' => $this->input->post("Redeem_points"),
					  'Redeem_amount' => $this->input->post("Redeem_points"),
					  'Manual_billno' => $this->input->post("manual_bill_no"),
					  'Trans_date' => $lv_date_time,
...
/**************************wallet debit*****************************/
	...
	public function validate_member_earn_code() 
	{
      $result = $this->Transactions_model->get_earn_code_info($this->input->post("Earn_code"), $this->input->post("Company_id"), $this->input->post("Membership_id"));
