  ...
  class Reportc extends CI_Controller {
    ...
    public function Customer_points_expiry() {
      if ($this->session->userdata('logged_in')) {
        ...
        $Company_id = $session_data['Company_id'];
        $data["Company_details"] = $this->Igain_model->get_company_details($session_data['Company_id']);
		$Points_expiry_period=$data["Company_details"]->Points_expiry_period;
        ...
        if ($_REQUEST != NULL) {
          ...
          //  $data["Count_Records"] = $this->Report_model->get_cust_expiry_report($lv_Company_id, $start_date, $end_date, '', '');
            ...
            $data["Trans_Records"] = $this->Report_model->get_cust_trans_summary_tobe_expiry_monthly($lv_Company_id,$Points_expiry_period, $days);
				...
				if ($pdf_excel_flag == 1) {
				  ...
				  foreach ($data["Trans_Records"] as $data1) {
					  ...
					  if($transaction_from==0 || $transaction_from==2)
						{
							...
							$POS_discount = $this->Report_model->Get_outlet_pos_discount_summary($Company_id, $start_date, $end_date, 2,$data1->Seller);
							if($POS_discount != NULL)
							{
								foreach($POS_discount as $dec)
								{
									$discount=$dec->Total_discount;
					  ...
					  if($data1->Channel == 12)
						...
						// $data1->Total_discount_amount = number_format(round($data1->Total_discount_amount),2);
						$data1->Total_discount_amount = number_format(round($Total_discount1),2);
	...
	{
		$result = $this->Report_model->get_cust_info($this->input->post("Cust_name"),$this->input->post("Company_id"));
		
		$result = array("Member_name"=>$result[0]["First_name"].' '.$result[0]["Last_name"],"Enrollement_id"=>$result[0]["Enrollement_id"],"Card_id"=>$result[0]["Card_id"],"User_email_id"=>App_string_decrypt($result[0]["User_email_id"]),"Phone_no"=>App_string_decrypt($result[0]["Phone_no"]));
      ...
      if ($this->session->userdata('logged_in')) {
        ...
        if ($_REQUEST != NULL) {
			...
			if(!$pdf_excel_flag)
			{
				...
				$Trans_Records = $this->Report_model->Get_voucher_report_records($Company_id, $start_date, $end_date, $Status,$Outlet_id,$Stamp_flag=0,$membership_id);
				$_SESSION['Trans_Records']=$Trans_Records;
				$data["Trans_Records"] = $Trans_Records;
			...
			if($pdf_excel_flag > 0)
			{
				...
				if ($pdf_excel_flag == 1) {
				  ...
				  foreach ($data["Trans_Records"][0] as $key => $field) {
					...
					if($key == "Payment_Type_id" || $key == "Update_date" || $key =="Seller_id" || $key =="Update_user_id" || $key =="Card_balance"){
						continue;
					}
					// $this->excel->getActiveSheet()->getStyle('E')->getNumberFormat()->setFormatCode('0.00');
					$fields[] = $key;
			...
			if($pdf_excel_flag > 0)
			{
				...
				if ($pdf_excel_flag == 1) {
				  ...
				  foreach ($data["Trans_Records"] as $data1) {
					...
					// $data1->No_of_Issued_Vouchers = $data1->Total_Issued_vouchers;
					
					
					$col = 0;
					foreach ($fields as $field) {
						$this->excel->setActiveSheetIndex(0)->setCellValue('C1', 'No_of_Issued_Vouchers');
						$this->excel->setActiveSheetIndex(0)->setCellValue('D1', 'No_of_Used_Vouchers');
						$this->excel->setActiveSheetIndex(0)->setCellValue('E1', 'No_of_Unused_Vouchers');
						$this->excel->setActiveSheetIndex(0)->setCellValue('F1', 'Total_Value_of_Vouchers_Used');
						$this->excel->setActiveSheetIndex(0)->setCellValue('G1', 'Total_Value_of_Unutilized_Vouchers');
					  $this->excel->getActiveSheet()->setCellValueByColumnAndRow($col, $row, $data1->$field);
					  $col++;
					}
					$row++;
	...
	public function get_items_by_category()
	{
		...
		// if($Category > '0')
		// {
			$data['Items'] = $this->Report_model->Get_Linked_Items_for_category($Seller_id,$Category,$Company_id);
      ...
      if ($this->session->userdata('logged_in')) {
        ...
        if ($_REQUEST != NULL) {
			...
			if(!$pdf_excel_flag)
			{
				...
				$Trans_Records = $this->Report_model->Get_voucher_report_records($Company_id, $start_date, $end_date, $Status,$Outlet_id,$Stamp_flag=1,$membership_id='');
				$_SESSION['Trans_Records']=$Trans_Records;
				$data["Trans_Records"] = $Trans_Records;
	...
	//****************Unclaimed Points 20-12-2021 AMIT KAMBLE**********************
	public function Unclaimed_points_report()
	{
		if ($this->session->userdata('logged_in')) {
        $session_data = $this->session->userdata('logged_in');
        $data['username'] = $session_data['username'];
		...
		$data['Seller_array'] = $get_sellers;
		 ...
		 if ($_REQUEST == NULL) {
			 $this->session->unset_userdata('SellersArrayReport2');
			 $data["report_type"] = 1;
			$this->load->view("Reports/Unclaimed_report_view", $data);
		 }
          ...
          //echo "dssfsdfsd";
			 ...
			 // $end_date = $_REQUEST["end_date"];
			  ...
			  $pdf_excel_flag = $_REQUEST["pdf_excel_flag"];
				...
				if(!$pdf_excel_flag)
				{
					$Trans_Records = $this->Report_model->get_uncliamed_points_report($Company_id, $start_date, $end_date, $transaction_from,$Outlet_id,$report_type,$Channel_id);
			...
			if($pdf_excel_flag > 0)
			{
				...
				if ($report_type == 1) {
				  	$Export_file_name = $cmp_name . "_Unclaimed_Points_Report_detail_report_" . $Today_date;
				} else {
				  	$Export_file_name = $cmp_name . "_Unclaimed_Points_summary_report_" . $Today_date;
				}
				...
				if ($pdf_excel_flag == 1) {
				  $this->excel->getActiveSheet()->setTitle('Unclaimed Points Report');
				  // $this->excel->stream($Export_file_name.'.xls', $data["Trans_Records"]);
				  $fields = array();
				  ...
				  $html = $this->load->view('Reports/pdf_Unclaimed_report_view', $data, true);
				  $this->m_pdf->pdf->WriteHTML($html);
				  $this->m_pdf->pdf->Output($Export_file_name . ".pdf", "D");
				}
			}
	...
	//****************Unclaim Points 20-12-2021 AMIT KAMBLE****XXX******************
	
	public function Customer_tier_report()
	{
      if ($this->session->userdata('logged_in')) {
		$session_data = $this->session->userdata('logged_in');
        $data['username'] = $session_data['username'];
        ...
        $Company_id = $session_data['Company_id'];
		 ...
		 if ($_REQUEST == NULL) {
			$this->load->view("Reports/Customer_tier_report_view", $data);
		 }
        ...
        if ($_REQUEST != NULL) {
			  ...
			  $end_date = $_REQUEST["end_date"];
			  $membership_id = $_REQUEST["membership_id"];
			  $Up_down_flag = $_REQUEST["Up_down_flag"];
			  ...
			  $pdf_excel_flag = $_REQUEST["pdf_excel_flag"];
				...
				$data['end_date'] = $_REQUEST["end_date"];
				$data['membership_id'] = $_REQUEST["membership_id"];
				$data['Up_down_flag'] = $_REQUEST["Up_down_flag"];
			
			if(!$pdf_excel_flag)
			{
				// unset($_SESSION['Trans_Records']);
				// session_destroy($_SESSION['Trans_Records']);
				 $data2["Records1"] = $this->Igain_model->get_customer_details_Card_id($membership_id, $Company_id);
				$Enrollement_id = $data2["Records1"]->Enrollement_id;
				
				$Trans_Records = $this->Report_model->Get_Customer_tier_report($Company_id, $start_date, $end_date, $Up_down_flag, $Enrollement_id);
			...
			if($pdf_excel_flag > 0)
			{
				...
				$cmp_name = str_replace(' ', '_', $data['Company_name']);
				$Export_file_name = $cmp_name . "Customer_tier_report" . $Today_date;

				$data["Report_date"] = $Today_date;
				
				if ($pdf_excel_flag == 1) {
				  $this->excel->getActiveSheet()->setTitle('Tier Upgrade-Downgrade REPORT');
				  // $this->excel->stream($Export_file_name.'.xls', $data["Trans_Records"]);
				  $fields = array();
				  ...
				  $html = $this->load->view('Reports/pdf_Customer_tier_report', $data, true);
				  $this->m_pdf->pdf->WriteHTML($html);
				  $this->m_pdf->pdf->Output($Export_file_name . ".pdf", "D");
				}
			}
	...
	//****************Loyalty summary report 12-2-2024 AMIT KAMBLE**********************
	public function Loyalty_summary_Report()
	{
      if ($this->session->userdata('logged_in')) {
        $session_data = $this->session->userdata('logged_in');
        $data['username'] = $session_data['username'];
		...
		$data["Channel_partner"] = $this->Report_model->get_company_Channel_partner_details($session_data['Company_id']);
		 ...
		 if ($_REQUEST == NULL) {
			$this->load->view("Reports/Loyalty_summary_report_view", $data);
		 }
        ...
        if ($_REQUEST != NULL) {
          //echo "dssfsdfsd";
			...
			if(!$pdf_excel_flag)
			{
				...
				$Trans_Records = $this->Report_model->Get_loyalty_summary_trans_records($Company_id, $end_date, $transaction_from,$Outlet_id,$Redemptionratio,$Channel_id);
				// die;
				$_SESSION['Trans_Records']=$Trans_Records;
				$data["Trans_Records"] = $Trans_Records;
			...
			if($pdf_excel_flag > 0)
			{
				...
				$cmp_name = str_replace(' ', '_', $data['Company_name']);
				$Export_file_name = $cmp_name . "_Loyalty_summary_report_" . $Today_date;
				...
				if ($pdf_excel_flag == 1) {
				  ...
				  foreach ($data["Trans_Records"] as $data1) {
						...
						else if($data1->Channel == 2)
						{
							$data1->Channel = "POS";
						}
						else if($data1->Channel == 29)
						{
							$Channel_name = $ci_object->Report_model->Get_beneficiary_company($data1->Channel_id);
							$data1->Channel = $Channel_name->Beneficiary_company_name;
						}
						else
						{
					...
					$data1->Earn_points = round($data1->Earn_points);
					$data1->Redeem_points = round($data1->Redeem_points);
					// $data1->Expired_points = round($data1->Expired_points);
					$data1->Loyalty_Points_Balance = round($data1->Loyalty_Points_Balance);
						$col = 0;
					foreach ($fields as $field) {
					  $this->excel->getActiveSheet()->setCellValueByColumnAndRow($col, $row, $data1->$field);
					  $col++;
				  ...
				  $html = $this->load->view('Reports/pdf_loyalty_summary_report', $data, true);
				  $this->m_pdf->pdf->WriteHTML($html);
				  $this->m_pdf->pdf->Output($Export_file_name . ".pdf", "D");
				}
			}
	...
	public function Company_Loyalty_summary_Report()
	{
      if ($this->session->userdata('logged_in')) {
        $session_data = $this->session->userdata('logged_in');
        $data['username'] = $session_data['username'];
		...
		$data["Channel_partner"] = $this->Report_model->get_company_Channel_partner_details($session_data['Company_id']);
		 ...
		 if ($_REQUEST == NULL) {
			$this->load->view("Reports/Company_Loyalty_summary_report_view", $data);
		 }
        ...
        if ($_REQUEST != NULL) {
          //echo "dssfsdfsd";
			...
			if(!$pdf_excel_flag)
			{
				...
				$Trans_Records = $this->Report_model->Get_Company_loyalty_summary_trans_records($Company_id, $end_date, $transaction_from,$Channel_id);
				// die;
				$_SESSION['Trans_Records']=$Trans_Records;
				$data["Trans_Records"] = $Trans_Records;
			...
			if($pdf_excel_flag > 0)
			{
				...
				$cmp_name = str_replace(' ', '_', $data['Company_name']);
				$Export_file_name = $cmp_name . "_Company_Loyalty_summary_report_" . $Today_date;

				$data["Report_date"] = $Today_date;
				$data["Company_name"] = $Company_name;
				$pdf_excel_flag = $_REQUEST["pdf_excel_flag"];
				if ($pdf_excel_flag == 1) {
				  $this->excel->getActiveSheet()->setTitle('Company Loyalty Report');
				  // $this->excel->stream($Export_file_name.'.xls', $data["Trans_Records"]);
				  $fields = array();
				  ...
				  $html = $this->load->view('Reports/pdf_Company_loyalty_summary_report', $data, true);
				  $this->m_pdf->pdf->WriteHTML($html);
				  $this->m_pdf->pdf->Output($Export_file_name . ".pdf", "D");
				}
			}
	...
	public function stamp_report()
	{
      if ($this->session->userdata('logged_in')) {
        ...
        $data['Symbol_of_currency'] = $Country_details->Symbol_of_currency;
		 ...
		 if ($_REQUEST == NULL) {
			$this->load->view("Reports/stamp_report_view", $data);
		 }
        ...
        if ($_REQUEST != NULL) {
			  ...
			  $end_date = $_REQUEST["end_date"];
			  $select_cust = $_REQUEST["select_cust"];
			  $card_id = $_REQUEST["Single_cust_membership_id"];
			  $tier_id = $_REQUEST["Tier_id"];
			  ...
			  $pdf_excel_flag = $_REQUEST["pdf_excel_flag"];
				...
				$data['start_date'] = $_REQUEST["start_date"];
				$data['end_date'] = $_REQUEST["end_date"];
				$data['select_cust'] = $_REQUEST["select_cust"];
				$data['card_id'] = $_REQUEST["Single_cust_membership_id"];
				$data['tier_id'] = $_REQUEST["Tier_id"];
			if(!$pdf_excel_flag)
			{
				unset($_SESSION['Trans_Records']);
				session_destroy($_SESSION['Trans_Records']);
				
				$Trans_Records = $this->Report_model->get_stamp_data($Company_id,$start_date,$end_date ,$card_id,$tier_id,$select_cust);
			...
			if($pdf_excel_flag > 0)
			{
				...
				$cmp_name = str_replace(' ', '_', $data['Company_name']);
				$Export_file_name = $cmp_name . "_Stamp_report_" . $Today_date;
				...
				$pdf_excel_flag = $_REQUEST["pdf_excel_flag"];
				if ($pdf_excel_flag == 1) {
				  $this->excel->getActiveSheet()->setTitle('Stamp Report');
				  // $this->excel->stream($Export_file_name.'.xls', $data["Trans_Records"]);
				  $fields = array();
				  ...
				  foreach ($data["Trans_Records"][0] as $key => $field) {
					...
					/* if($key == "Trans_date" || $key == "Card_id" || $key == "Member_name" || $key =="Bill_no" || $key =="Purchase_amt"){
						continue;
					} */
					// $this->excel->getActiveSheet()->getStyle('E')->getNumberFormat()->setFormatCode('0.00');
					$fields[] = $key;
				  ...
				  $html = $this->load->view('Reports/pdf_stamp_report', $data, true);
				  $this->m_pdf->pdf->WriteHTML($html);
				  $this->m_pdf->pdf->Output($Export_file_name . ".pdf", "D");
				}
			}
