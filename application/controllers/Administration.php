...
//**************** sandeep work start ***************************************	
	...
	function communication()
	{
		if($this->session->userdata('logged_in'))
		{
          ...
          /*   $params['data'] =$data['enroll'];
            $params['level'] = 'H';
            $params['size'] = 5;
            $params['savename'] = FCPATH . 'VoucherQRcodes/' . $data['enroll'] . 'enroll.png';
			...
			if($_POST == NULL)
			{
				...
				$data['All_customers'] = $this->Igain_model->get_all_customers($data['Company_id']);		
			...
			{			
				...
				{
					$seller_merchant = $this->input->post('seller_id');
					foreach($seller_merchant as $merchant)
					{
						/*-----------------------File Upload---------------------*/
						...
						$result = $this->Administration_model->insert_communication($filepath,$merchant);
					}
					if($result == true)
					{
						...
						/****************Nilesh log tbl change******************/	
							...
							$Company_id	= $session_data['Company_id'];
							$get_seller_detail = $this->Igain_model->get_enrollment_details($merchant);
							...
							$result_log_table = $this->Igain_model->Insert_log_table($Company_id,$enroll,$username,$Seller_name,$Todays_date,$what,$where,$userid,$opration,$opval,$firstName,$lastName,$merchant);
				...
				if( $this->input->post("submit") == "Send" )
				{
					...
					$Brand_name = $seller_details->First_name.' '.$seller_details->Middle_name;
					...
					{
						if($Enrollment_id > 0)   //****single customer
						{
							...
							else
							{
								...
								/*----------------------Product & Revenue Voucher-02-03-2020-------------------*/
									...
									if($Link_to_voucher ==1){
										...
										$getVoucherMaster=$this->Voucher_model->Get_voucher_details($Voucher_type,$Company_id,$Todays_date);
										
										 // echo"---getVoucherDetails----".$this->db->last_query()."---<br>";
										// var_dump($getVoucherDetails);Get_voucher_details
										// die;
										
												
												//-----------------SEND VOUCHERS COMMUNICATION PDF--XXX----------------
												// print_r($getVoucherDetails);die;
										if($getVoucherDetails != ''){
											  //---------------------Create Zip --------------//
										  $Filename=$data['Company_name'].'-Vouchers_List-'.date('Y_m_d_H_i');
												$zip = new ZipArchive();
										
												$FilenameZip="$Filename.zip";

												if(file_exists('Vouchers_list/'.$FilenameZip)) {
													unlink ('Vouchers_list/'.$FilenameZip); 

												}
												if ($zip->open('Vouchers_list/'.$FilenameZip, ZIPARCHIVE::CREATE) != TRUE) {
														die ("Could not open archive");
												}
												$VouchersList_ZIP='Vouchers_list/'.$Filename.'.zip';
											for($a=0;$a<$_REQUEST['VoucherQuantity'];$a++)
											{	
												...
												$Discount_value =$getVoucherDetails[0]['Cost_price'];
												$data['Item_image1'] =$getVoucherMaster[0]['Item_image1'];
												
												$data['Voucher_description'] =$getVoucherMaster[0]['Voucher_description'];
												...
												foreach($getVoucherDetails as $vouchers){
													...
													/* $len = strlen($customer_details->Card_id);
													$Fromlen = $len-3;
													$Card_idLast = substr($customer_details->Card_id, $Fromlen, $len);
													$Voucher_no = $Card_idLast.'-'.$Voucher_no; */
													$Voucher_array[]=$Voucher_no;													
													$data['Voucher_no']=$Voucher_no;										
													$data['Valid_from']=$vouchers['Valid_from'];							
													$data['Valid_till']=$vouchers['Valid_till'];							
													$data['Cost_price']=$vouchers['Cost_price'];							
													$data['Cost_in_points']=$vouchers['Cost_in_points'];
													$data['Discount_percentage']=$vouchers['Discount_percentage'];
													$data['Voucher_type']=$vouchers['Voucher_type'];
													...
													if($LV_Voucher_no!=$Voucher_no)
													{
														...
														//-----------------SEND VOUCHERS COMMUNICATION PDF------------------
														$Get_voucher_child_datails=$this->Voucher_model->Get_voucher_child_datails($vouchers['Voucher_id']);
														if($Get_voucher_child_datails !=NULL)
														{
															foreach($Get_voucher_child_datails as $recV)
															{
																$Merchandize_item_name[] = $recV['Merchandize_item_name'];
															}
															$data['ProductVoucherItems'] = $Merchandize_item_name;
														}
														$data['Customer_name'] = $customer_details->First_name.' '.$customer_details->Last_name;
														$data['CustomerCard_id'] = $customer_details->Card_id;
														// echo $data['Item_image1'];die;
														$html = $this->load->view('administration/pdf_send_vouchers_communication', $data, true);
														// ECHO $html;die;
														...
														$pdf->WriteHTML($html);
														$pdf->Output('Vouchers_list/'.$Filename.'-'.$Voucher_no.'.pdf','F');
														$VouchersList_PDF='Vouchers_list/'.$Filename.'-'.$Voucher_no.'.pdf';
														
														  //---------------------Create Zip --------------//
													
															$zip->addFile($VouchersList_PDF);
															
													
														// close and save archive
														//-----------------SEND VOUCHERS COMMUNICATION PDF--XXX----------------
														$LV_Voucher_no=$Voucher_no;
												}
								...
								$Email_content = array(
									...
									'Voucher_array' => $Voucher_array,
									'Voucher_type' => $Vouchertype,
									'VouchersList_PDF' => $VouchersList_ZIP,
									'VoucherQuantity' => $_REQUEST['VoucherQuantity'],
									'Template_type' => 'Communication'
								);						
								$send_notification = $this->send_notification->send_Notification_email($customer_details->Enrollement_id,$Email_content,$sellerid,$compid);
						...
						if($radio2 == 2)   //****all customer
						{
							...
							$all_customers = $sendto;	
							// foreach ($all_customers as $row)
							// print_r($all_customers); die;
							foreach ($all_customers as $SelectedEnrollement_id)
							{
								$cust_emails[$i++] = $row['User_email_id'];
								//$customer_details = $this->Igain_model->get_enrollment_details($row['Enrollement_id']);	
								
								$customer_details = $this->Igain_model->get_enrollment_details($SelectedEnrollement_id);
								
								$cust_emails[$i++] = $customer_details->User_email_id;						
	...
	public function get_offer_details()
	{
		...
		$this->output->set_content_type('application/json');
		$this->output->set_output(json_encode(array('offer_description'=> $result->description,'offer_hobbie_id' => $result->Hobbie_id,'Link_to_voucher' => $result->Link_to_voucher)));
		if($result->Link_to_voucher == 1)
		{
			$voucher_datails = $this->Administration_model->Get_voucher_datails($result->Voucher_id);
			$Voucher_type = $voucher_datails->Voucher_type;
			$Voucher_id = $voucher_datails->Voucher_id;
			$Voucher_name = $voucher_datails->Voucher_name;
			$Valid_from = $voucher_datails->Valid_from;
			$Valid_till = $voucher_datails->Valid_till;
			
			if($Voucher_type==1){  $Voucher_type="Revenue Voucher";}
			if($Voucher_type==2){  $Voucher_type="Product Voucher";}
			if($Voucher_type==3){  $Voucher_type="Discount Voucher";}
		//	$this->output->set_output($result->description);
			$Todays_date = date('Y-m-d H:i:s');	
			$Voucher_valid_flag=0;
			if(($Todays_date >= $Valid_from) && ($Todays_date <= $Valid_till))
			{
				$Voucher_valid_flag=1;
			}
			$this->output->set_output(json_encode(array('offer_description'=> $result->description,'offer_hobbie_id' => $result->Hobbie_id,'Link_to_voucher' => $result->Link_to_voucher,'Voucher_id' => $Voucher_id,'Voucher_name' => $Voucher_name,'Voucher_type' => $Voucher_type,'Voucher_valid_flag' => $Voucher_valid_flag)));
		}
...
//******* offer rule sandeep 18-05-2020 *************
		...
		if($this->session->userdata('logged_in'))
		{
			...
			{			
				...
				/*-----------------------------------Image1-----------------------------*/
					...
					$configThumb = array();
					$configThumb['image_library'] = 'gd2';
					$configThumb['max_size'] = '200';
					$configThumb['size'] = '200';
					$configThumb['source_image'] = '';
					$configThumb['create_thumb'] = TRUE;
					$configThumb['maintain_ratio'] = TRUE;
			...
			{		
				...
				foreach($data["results3"] as $row)
				{
					...
					if($this->upload->display_errors())
					{
						...
						{
							$this->session->set_flashdata("error_code",$this->upload->display_errors());
							redirect('Administration/Upload_Special_Offers', 'refresh');	
						}
	...
	function validate_member() {
      $result = $this->Igain_model->validate_member($this->input->post("cardid"), $this->input->post("Company_id"));
