...
class Auto_Process_points_expiry_Monthly_ONE extends CI_Controller 
{
	...
	public function index()
	{
		...
		foreach($Company_details as $Company_Records)
		{
			...
			// die;
			//-------------------------Flow 3: Get All Customers----------------------------------
			
			$All_custExpiry_Summary=$this->Auto_process_model->get_cust_trans_summary_custmers($Company_Records["Company_id"]);
			if($All_custExpiry_Summary != NULL)
			{		
				foreach($All_custExpiry_Summary as $Cust_RecordSummary)
				{
					 /*  if($Cust_RecordSummary->Enrollement_id != '10418')// 
					{
						continue;
					}  */
					$Expire_pts=0;
					$TobeExpire_pts=0;
					$Availabel_Current_balance=0;
					
					//-------------------------Flow 3.1: Logic to Expire Points----------------------------------
					$Expiry_Summary=$this->Auto_process_model->get_cust_trans_summary_expiry_monthly($Cust_RecordSummary->Enrollement_id);
					...
					// die; 
						...
						{		
							foreach($Expiry_Summary as $Trans_Records)
							{
								...
								$Expire_pts=round($Trans_Records->Expire_pts) ;
								...
								if($Expire_pts>0 && $Total_Current_balance > 0) 
								{
									...
									/******************Update ExpiryPoints to Knockout_points*******************************/
									$Gained_Summary=$this->Auto_process_model->get_cust_gained_points_trans_summary($Enrollement_id);
									if($Gained_Summary != NULL)
									{
										$Expire_pts2= $Expire_pts;
										foreach($Gained_Summary as $Gained_SummaryRec)
										{
											if($Expire_pts2 > 0)
											{
												$diff = (($Gained_SummaryRec['Total_topup_amount']+$Gained_SummaryRec['Total_loyalty_pts'])-$Gained_SummaryRec['Knockout_points']);
												echo "<br><br>diff $diff     Expire_pts $Expire_pts ";
												...
												if($diff < $Expire_pts2)
												{
													...
													$lv_knock_pts=$Gained_SummaryRec['Knockout_points']+$diff;
													$UpdateKnockout_points=$this->Auto_process_model->Update_Cust_Knockout_points_trans_summary($Gained_SummaryRec["Trans_summ_id"],$lv_knock_pts);
													$Expire_pts2 = $Expire_pts2 - $diff;
									
												}
												else
												{
													$lv_knock_pts=$Gained_SummaryRec['Knockout_points']+$Expire_pts2;
													$UpdateKnockout_points=$this->Auto_process_model->Update_Cust_Knockout_points_trans_summary($Gained_SummaryRec["Trans_summ_id"],$lv_knock_pts);
													$Expire_pts2 =0;
												}	
						...
						$ExpiryTOBE_Summary=$this->Auto_process_model->get_cust_trans_summary_tobe_expiry_monthly($Cust_RecordSummary->Enrollement_id);
						if($ExpiryTOBE_Summary != NULL)
						{	
							foreach($ExpiryTOBE_Summary as $Trans_Records2)
							{
								$TobeExpire_pts = 	round($Trans_Records2->TobeExpire_pts);
								if($Availabel_Current_balance==0){$Availabel_Current_balance=($Trans_Records2->Current_balance-$Trans_Records2->Blocked_points);}
								
							}
						}
						
						//-----------------Flow 5: Send Notification-------------------------------------------------
						if($Expire_pts >0 || $TobeExpire_pts > 0)
						{
							$Email_content = array(
							'TobeExpire_pts' => $TobeExpire_pts,
							'Expire_pts' => $Expire_pts,
							'Availabel_Current_balance' => $Availabel_Current_balance,
							'Notification_type' => 'Notification: Expiry of Loyalty Points',
							'Template_type' => 'Points_Expiry_JAVA'
							);
							echo "<br><br><h1>Expired Pts $Expire_pts  TobeExpire Pts $TobeExpire_pts</h1>";
							$Notification=$this->send_notification->send_Notification_email($Cust_RecordSummary->Enrollement_id,$Email_content,'1',$Company_Records["Company_id"]);
						}
