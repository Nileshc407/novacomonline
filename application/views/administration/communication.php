				...
				<div class="col-sm-6">					
					...
					<div class="form-group">
						...
						<label class="form-check-label">
							<input type="radio" class="form-check-input comm_type" name="r1" id="r1" value="1" onclick="set_merchant(this.value);">Create New Communication							
						</label>
						 </div> 
						<div class="form-check" >
						<label class="form-check-label">								
							<input type="radio"  class="form-check-input comm_type" name="r1" id="r2" value="2" onclick="set_merchant(this.value);">Send Existing Communication
						...
						<label class="form-check-label">
							...
							<input type="radio" name="r1" class="form-check-input comm_type" id="r3" value="3" onclick="set_merchant(this.value);">Send Multiple Communication <!-- data-toggle="modal" data-target="#myModal"  -->
						</label>
						 </div> 
						 <div class="help-block form-text with-errors form-control-feedback" id="comm_type" ></div>
					</div>	
					<div class="form-group" id="single_seller">
					...
					<div class="form-group" style="display:none;" id="multi_seller">
						<label for=""><span class="required_info">*</span> Merchant Name</label>
							<select class="form-control" name="seller_id[]" id="seller_id"  multiple>
							<?php
							if($Logged_user_id > 2 || $Super_seller == 1)
							{
								echo '<option value="'.$enroll.'">'.$LogginUserName.' (Company Admin) '.'</option>';									
						...
						<legend><span>Create New Communication</span></legend>							
							...
							<?php if($Company_details->Cron_communication_flag  == 1 ){  ?>
								<div class="form-group" id="schedule_communication_flag">
									...
									&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
									<label class="form-check-label">
										<input type="radio" class="form-check-input"  name="Schedule_flag" id="Schedule_flag" <?php //if($Company_details->Cron_communication_flag  == 0 ){echo 'disabled'; } ?> onclick="hide_show_criteria(this.value)" value="1">Yes	required							
									</label>
									&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
									<label class="form-check-label">										
										<input type="radio" class="form-check-input"  name="Schedule_flag" id="Schedule_flag" onclick="hide_show_criteria(this.value)" value="0" checked >No									
							...
							<!---------------------Schedule Communication--------------------------------->
								...
								<div class="form-group" id="Link_to_lbs_div" style="display:none;">								
										...
										<div class="row">
											...
											<div class="col-md-6">									
												<div class="row">	
													<div class="col-md-12">	
														<label class="form-check-label">
															<input type="radio" name="Voucher_type" class="form-check-input"  onclick="hide_show_voucher_type(1);get_vouchers(1);" id="Voucher_type" value="1">Revenue Voucher								
														</label>
													</div>
													<div class="col-md-12">	
														<label class="radio-inline">										
					...
					<div class="form-group" id="send_exist_offer">
						...
						<div class="form-group">
							...
							<label class="form-check-label" style="margin-left:10px;">
								...
								<div class="form-group" id="Link_to_voucher_div1" style="display:none;">								
								<div class="form-group" >								
										<label for=""> Link to Voucher </label>
										&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
										&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
										...
										&nbsp;&nbsp;
										<label class="form-check-label">
											<input type="radio" name="Link_to_voucher1" class="form-check-input"   id="Link_to_voucher1" value="1" checked>Yes								
										</label>
										&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
										<label class="radio-inline">										
											<input type="radio" name="Link_to_voucher1"  class="form-check-input"  id="Link_to_voucher1"  value="0" DISABLED  >No									
										</label>
									</div>
									...
									<div class="form-group" id="Product_voucher_div1">
										<legend style="color: black;"><span id="voucher_type1">Product Voucher:</span></legend>
										<div class="form-group">								
											<select class="form-control" name="Voucher_id1" id="Voucher_id1" readonly>
												
											</select>								
										</div>
										<div id="VouchervalidityError" style="color: red;display:none">Note: Linked voucher validity has expired !!!</div>	
									</div>
									
									<div class="form-group" id="block1" >
										<label for=""><span class="required_info">*</span> Quantity :</label>
										<input type="text" id="VoucherQuantity" name="VoucherQuantity" class="form-control"  placeholder="Enter Quantity" value="1"   onkeyup="this.value=this.value.replace(/\D/g,'')" />								
										<div id="VoucherQuantityError" style="color: red;display:none">Please enter Quantity between 1 and 100</div>		
									</div>
								</div>
					...
					<div class="panel panel-default" id="send_to_single">
						<legend><span>Send to Single Member</span></legend>
							...
							<div class="form-group" id="block1" >
							<label for=""><span class="required_info">*</span> Membership ID / Mobile No./ Member Name</label>
							<input type="text" id="mailtoone_memberid" name="mailtoone_memberid" class="form-control"  placeholder="Enter Membership ID / Mobile No./ Member Name" value="" autocomplete="off">								
					...
					<div class="form-group" id="send_to_all">
						<legend><span>Send to All Members:</span></legend>
							<div class="form-group">								
								<select class="form-control" name="mailtoall[]" multiple>
									<?php 
										
										foreach($All_customers as $rec)
										{
											$Full_name=$rec['First_name']." ".$rec['Last_name'];
											echo "<option value='".$rec['Enrollement_id']."' >".$Full_name."</option>";
...
$('#Send').click(function()
{
	if( $('#companyId').val() != "" && $('#sellerId').val() != "" )
	{
		...
		// alert('VoucherQuantity '+VoucherQuantity);
			if(VoucherQuantity== "" || VoucherQuantity <= 0 || VoucherQuantity > 100)
			{
				$("#VoucherQuantityError" ).show();
				$("#VoucherQuantity" ).val('');
				$("#VoucherQuantity").attr("required","required");
				 r1Value = 99;
			}
...
{	
	...
	$('#activeoffer').change(function()
	{
		...
		$.ajax({
			...
			{
				$("#Link_to_voucher_div1").hide();
				$("#VouchervalidityError").hide();
				$( "#Send" ).show();
				// $('#offerdetails').val(data);Link_to_voucher
				...
				if(json['Link_to_voucher']==1)
				{
					// alert(json['Voucher_id']);
					$("#Link_to_voucher_div1").show();
					$("#VoucherQuantity").val('1');
					$('#voucher_type1').html(json['Voucher_type']);
					$('#Voucher_id1').html("<option value='"+json['Voucher_id']+"'>"+json['Voucher_name']+"</option>");
					 if(json['Voucher_valid_flag']==0)
					 {
						 $("#VouchervalidityError").show();
						 $( "#Send" ).hide();
					 }						 
				}
	...
	/*********************************Autocomplete***************************************/
		$("#mailtoone_memberid").autocomplete({
			source: "<?php echo base_url()?>index.php/Administration/autocomplete_customer_names" // path to the get_birds method
		});
	/*********************************Autocomplete***************************************/
	
	function get_phnno_memberid(Cust_name)
	{
		alert(Cust_name);
		// var Cust_name = $("#mailtoone_memberid").val();	
		var Company_id = '<?php echo $Company_id; ?>';
		if( $("#mailtoone_memberid").val() == "")
		{
			$("#mailtoone_phnno").val("");				
			$("#mailtoone_memberid").val("");				
			...
			$.ajax({
				...
				success: function(json)
				{
					...
					else
					{
						...
						$("#MemberEnrollmentId").val(json[0].Enrollement_id);				
					}
				}
			});
		} 
			...
			$.ajax({
				...
				success: function(json)
				{
					...
					else
					{
						...
						$("#mailtoone_memberid").val(json['Card_id']);				
...
function set_merchant(inputVal)
{
	if(inputVal == 1)
	{
		$("#single_seller").hide();
	...
	else if(inputVal == 2)
	{
		...
		$("#sellerId").attr("required","required");
	}
}
</script>
<!-- ************************************AM Graph************************************* --
