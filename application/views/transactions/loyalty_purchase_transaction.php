            ...
            <div class="col-sm-7">
                <div class="element-wrapper">
                    ...
                    <div class="element-box">
						...
						<?php if($Use_member_code_as_card_id == 1) { ?>
						<div class="form-group">
                            <label for="">Customer Earn Code</label>
                            <input class="form-control" data-error="Enter Customer Earn Code" placeholder="Enter Customer Earn Code" required="required" type="text" id="Earn_code" name="Earn_code" class="form-control">
							  <div class="help-block form-text with-errors form-control-feedback" id="EarnCode"></div>
                        </div>
						<?php } ?>
						 <div class="form-group">
                            <label for=""> <span class="required_info">*</span> Outlet Name </label>
                            <select class="form-control" name="Outlet_id" id="Outlet_id" onchange="check_bill(this.value)" required>
							<option value="">Select Outlet</option>
								<?php
								  foreach ($Sub_sellers as $sellers) {
									echo "<option value=".$sellers->Enrollement_id.">".$sellers->First_name.' '.$sellers->Last_name."</option>";
								  }
								?>
                            </select>
                        </div>
...
//***** bill no validation **********/
  ...
  $('#manual_bill_no').blur(function ()
  {
	  ...
	  {
		  $("#manual_bill_no").val("");
          $("#BillNO").html("Please select outlet");
	  }
	  else
      {
          $.ajax({
              type: "POST",
              data: {Bill_no: Bill_no, Company_id: Company_id,Outlet_id:Outlet_id},
              url: "<?php echo base_url() ?>index.php/Transactionc/check_outlet_bill_no",
...
//***** bill no validation **********/
  ...
  $('#Earn_code').blur(function ()
  {
      var Earn_code = $('#Earn_code').val();
      var MemberCardID = '<?php echo $MembershipID; ?>';
      var Company_id = '<?php echo $Company_id; ?>';   
	  
      if ($('#Earn_code').val() == "") 
	  {
          $('#EarnCode').html("Enter Customer Earn Code");
          $("#Earn_code").addClass("form-control has-error");
      } 
	  else 
      {
          $.ajax({
              type: "POST",
              data: {Earn_code: Earn_code, Company_id: Company_id,Membership_id:MemberCardID},
              url: "<?php echo base_url() ?>index.php/Transactionc/validate_member_earn_code",
              success: function (data)
              {
                  if (data == 0)
                  {
                      $('#Earn_code').val("");
                      $('#EarnCode').html("Enter Valid Code..!");
                      $("#Earn_code").addClass("form-control has-error");
                  } 
				  else
                  {
                      json = eval("(" + data + ")");
                      if ((json[0].pinno_used) != 1)
                      {
                          $('#EarnCode').html("");
                          $("#Earn_code").removeClass("has-error");
                      } 
					  else
                      {
                          $('#Earn_code').val("");
                          $('#EarnCode').html("Customer Earn Code is Already Used..!");
                          $("#Earn_code").addClass("form-control has-error");
                      }
                  }
              }
          ...
          $.ajax({
              ...
              success: function (data)
              {
                  ...
                  } else
                  {
                      ...
                      json = eval("(" + data + ")");
                      // if ((json[0].Card_balance) != 0 && MemberCardID == json[0].MembershipID)
                      if (MemberCardID == json[0].MembershipID)
                      {
