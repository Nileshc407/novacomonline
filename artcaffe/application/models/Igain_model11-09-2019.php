	...
	function Fetch_Transaction_Detail_Reports($Company_id,$startDate,$endDate,$Merchant,$Trans_Type,$Report_type,$Enrollment_id,$membership_id,$Redeemption_report,$start,$limit)
	{
		...
		else
		{
			...
			{ 
				...
				{ 
					// echo"here.........";
					$this->db->select("igain_transaction.Trans_date AS TransactionDate,igain_transaction.Bill_no AS BillNo,igain_transaction.Manual_billno AS Manual_billno,igain_transaction_type.Trans_type AS TransactionType,igain_transaction.Item_code as Item_name,Item_size,Quantity,igain_transaction.Purchase_amount AS PurchaseAmount,igain_transaction.Paid_amount AS Paid_amount,igain_transaction.Shipping_cost as Shipping_cost,igain_transaction.Topup_amount AS BounsPoints,igain_transaction.Redeem_points AS RedeemPoints,igain_transaction.Loyalty_pts AS GainPoints,igain_transaction.Seller_name AS DoneBy,igain_transaction.Transfer_points AS TransferPoints,igain_transaction.Card_id2 as TransferToFrom,igain_transaction.Remarks as Remarks,igain_transaction_type.Trans_type_id AS TransactionTypeID");
					$this->db->from('igain_transaction');
					$this->db->where('igain_transaction.Trans_date >=', $startDate);
					$this->db->where('igain_transaction.Trans_date <=', $endDate);
					// $this->db->where('igain_transaction.Trans_type != 10 ');
